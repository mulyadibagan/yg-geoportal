...
def parse_capture_time(value):
    ...
    tz=m.group(3) or ''
    if tz and tz != 'Z' and ':' not in tz: tz=tz[:3]+':'+tz[3:]
...
def apply_metadata(job_path, summary_path):
    ...
    if re.match(r'^Survei drone\s+\d{1,2}[/-]\d{1,2}[/-]\d{4}$', title, re.I): d['title']='Survei drone'
    save(job_path,d)

def main():
    p=argparse.ArgumentParser(); sub=p.add_subparsers(dest='cmd',required=True)
    a=sub.add_parser('processing'); a.add_argument('job')
    a=sub.add_parser('stage'); a.add_argument('job'); a.add_argument('stage'); a.add_argument('progress',type=int); a.add_argument('label'); a.add_argument('--valid',type=int); a.add_argument('--excluded',type=int); a.add_argument('--downloaded',type=int); a.add_argument('--expected',type=int)
    ...
    if args.cmd=='processing':
        ...
        if args.downloaded is not None: extra['downloadedPhotos']=args.downloaded
        if args.expected is not None: extra['expectedPhotos']=args.expected
