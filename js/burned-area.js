(function(){
  ...
  fetch('data/burned-area-estimates.geojson?v='+Date.now(),{cache:'no-store',signal:AbortSignal.timeout(30000)}).then(function(r){if(!r.ok)throw Error('data');return r.json()}).then(function(geo){
    ...
    function details(p){return '<strong>'+esc((p.villages||[]).join(', ')||'Lokasi belum teridentifikasi')+'</strong><br>'+esc((p.regencies||[]).join(', '))+'<br><strong>'+hectares(p.estimatedAreaHa)+'</strong> · estimasi per kejadian<br>Deteksi: '+date(p.firstDetection)+' – '+date(p.lastDetection)+'<br>'+Number(p.hotspotCount||0)+' hotspot · '+Number(p.detectionDays||0)+' hari deteksi<br>Citra pascakejadian: '+esc((p.postSceneDates||[]).map(date).join(', ')||'—')+villageDetails(p)+'<br><small>Estimasi citra satelit, bukan verifikasi lapangan.</small>'}
