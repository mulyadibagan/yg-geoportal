(function(){
  ...
  const state={selected:LOCATIONS[0],product:'wave',data:new Map(),markers:new Map(),chart:null,areas:null,waveArrow:null,currentArrow:null,changeLayer:null,changeSummary:new Map(),rehabilitationSummary:new Map()};
  ...
  function renderRehabilitation(){const row=state.rehabilitationSummary.get(state.selected.name.toLowerCase()),box=$('rehabilitation-summary');if(!row){box.innerHTML='<strong>Prioritas rehabilitasi mangrove</strong><p>Hasil penilaian belum tersedia untuk lokasi ini.</p>';setTimeout(()=>map.invalidateSize(),0);return}box.innerHTML=`<strong>Prioritas rehabilitasi mangrove</strong><p>${row.recommendedAction||'Tinjau kandidat rehabilitasi pada peta prioritas.'}</p><div class="rehab-metrics"><span>Luas prioritas<b>${fmt(row.priorityAreaHa||row.candidateAreaHa,2)} ha</b></span><span>Polygon kandidat<b>${row.priorityPolygonCount||0}</b></span><span>Kebutuhan<b>${row.needScore||0}/100 · ${row.needClass||'—'}</b></span><span>Kelayakan<b>${row.suitabilityScore||0}/100 · ${row.suitabilityClass||'—'}</b></span></div><a href="mangrove-priority.html?regency=${state.selected.regency.toLowerCase()}&village=${state.selected.id}">Buka polygon prioritas →</a>`;setTimeout(()=>map.invalidateSize(),0)}
  function setSelected(loc){state.selected=loc;renderTabs();const data=state.data.get(loc.id);$('location-name').textContent=loc.name;$('location-meta').textContent=`Kabupaten ${loc.regency} · titik model laut terdekat`;renderCoastalChange();renderRehabilitation();if(data)renderLocation(data);map.setView([loc.lat,loc.lon],11);}
  ...
  async function loadRehabilitation(){try{const files=['data/mangrove-priority-bengkalis-results.json','data/mangrove-priority-siak-results.json'],results=await Promise.all(files.map(file=>fetch(file+'?v=20260813').then(response=>response.json())));results.flatMap(result=>result.villages||[]).forEach(row=>state.rehabilitationSummary.set(row.village.toLowerCase(),row));renderRehabilitation()}catch(e){console.warn(e);renderRehabilitation()}}
  const safeText=value=>String(value??'').replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));
  const uniqueSorted=values=>[...new Set(values.filter(Boolean))].sort((a,b)=>a.localeCompare(b,'id'));
  function filteredPreliminaryRows(){const regency=$('analysis-regency').value,district=$('analysis-district').value,village=$('analysis-village').value;return state.preliminarySummary.filter(row=>(!regency||row.regency===regency)&&(!district||row.district===district)&&(!village||row.village===village))}
  function populateAnalysisFilters(changed){const regency=$('analysis-regency'),district=$('analysis-district'),village=$('analysis-village');if(!changed){uniqueSorted(state.preliminarySummary.map(row=>row.regency)).forEach(value=>regency.add(new Option(value,value)))}if(!changed||changed==='regency'){const current=district.value;district.innerHTML='<option value="">Semua kecamatan</option>';uniqueSorted(state.preliminarySummary.filter(row=>!regency.value||row.regency===regency.value).map(row=>row.district)).forEach(value=>district.add(new Option(value,value)));if([...district.options].some(option=>option.value===current))district.value=current}if(!changed||changed==='regency'||changed==='district'){const current=village.value;village.innerHTML='<option value="">Semua desa</option>';uniqueSorted(state.preliminarySummary.filter(row=>(!regency.value||row.regency===regency.value)&&(!district.value||row.district===district.value)).map(row=>row.village)).forEach(value=>village.add(new Option(value,value)));if([...village.options].some(option=>option.value===current))village.value=current}}
  function focusPreliminaryVillage(name){const group=state.preliminaryVillageLayers.get(String(name).toLowerCase());if(group&&group.getBounds().isValid()){map.fitBounds(group.getBounds().pad(.35));if(!map.hasLayer(state.preliminaryLayer)){state.preliminaryLayer.addTo(map);$('toggle-preliminary-change').checked=true}}}
  function renderVillageAnalysis(row){const box=$('analysis-village-detail');if(!row){box.innerHTML='<strong>Pilih desa</strong><p>Klik baris desa untuk melihat metrik analisis lengkap.</p>';return}if(row.status!=='analysed'){box.innerHTML=`<p class="eyebrow">${safeText(row.regency||'')}</p><h3>${safeText(row.village)}</h3><span class="analysis-badge muted">Belum terkonfirmasi pesisir</span><p>${safeText(row.reason||'Data belum memadai.')}</p>`;return}box.innerHTML=`<p class="eyebrow">${safeText(row.regency)} · ${safeText(row.district)}</p><h3>${safeText(row.village)}</h3><span class="analysis-badge ${safeText(row.confidence)}">Keyakinan ${safeText(row.confidence)}</span><dl><div><dt>Indikasi kehilangan</dt><dd>${fmt(row.erosionAreaHa,2)} ha</dd></div><div><dt>Indikasi pertambahan</dt><dd>${fmt(row.accretionAreaHa,2)} ha</dd></div><div><dt>Laju kemunduran</dt><dd>${fmt(row.indicativeRetreatRateMPerYear,2)} m/tahun</dd></div><div><dt>Panjang pantai</dt><dd>${fmt(row.coastlineLengthKm,2)} km</dd></div><div><dt>Cakupan bebas awan</dt><dd>${fmt(row.clearCoveragePct,1)}%</dd></div><div><dt>Citra 2016 / 2025</dt><dd>${row.baselineSceneCount||0} / ${row.currentSceneCount||0}</dd></div><div><dt>Ketidakpastian posisi</dt><dd>±${fmt(row.positionalUncertaintyM,1)} m</dd></div><div><dt>Perubahan bersih</dt><dd>${fmt(row.netAreaChangeHa,2)} ha</dd></div></dl><button type="button" class="focus-village" data-focus-village="${safeText(row.village)}">Tampilkan polygon desa</button><small>Indikasi satelit, bukan penetapan abrasi. Verifikasi lapangan tetap diperlukan.</small>`}
  function renderAnalysisExplorer(){const rows=filteredPreliminaryRows(),analysed=rows.filter(row=>row.status==='analysed'),regencies=uniqueSorted(rows.map(row=>row.regency)).length,erosion=analysed.reduce((sum,row)=>sum+(row.erosionAreaHa||0),0),accretion=analysed.reduce((sum,row)=>sum+(row.accretionAreaHa||0),0);$('analysis-summary').innerHTML=`<article><span>Wilayah</span><strong>${regencies}</strong><small>kabupaten</small></article><article><span>Desa tersaring</span><strong>${rows.length}</strong><small>${analysed.length} dianalisis</small></article><article><span>Indikasi kehilangan</span><strong>${fmt(erosion,2)} ha</strong><small>total desa tersaring</small></article><article><span>Indikasi pertambahan</span><strong>${fmt(accretion,2)} ha</strong><small>total desa tersaring</small></article>`;$('analysis-table-body').innerHTML=rows.map(row=>`<tr data-village-row="${safeText(row.village)}"><td><strong>${safeText(row.village)}</strong><small>${safeText(row.district||'—')}</small></td><td>${safeText(row.regency||'—')}</td><td>${row.status==='analysed'?'Dianalisis':'Belum terkonfirmasi'}</td><td>${row.status==='analysed'?fmt(row.erosionAreaHa,2)+' ha':'—'}</td><td>${row.status==='analysed'?fmt(row.accretionAreaHa,2)+' ha':'—'}</td><td><span class="analysis-badge ${safeText(row.confidence||'muted')}">${safeText(row.confidence||'—')}</span></td><td><button type="button" data-detail-village="${safeText(row.village)}">Detail</button></td></tr>`).join('')||'<tr><td colspan="7">Tidak ada desa untuk filter ini.</td></tr>';if(rows.length===1)renderVillageAnalysis(rows[0])}
  async function loadPreliminaryCoastalChange(){try{const [summaryResponse,geoResponse]=await Promise.all([fetch('data/coastal-change-non-intervention-annual.json?v=20260812-meranti1'),fetch('data/coastal-change-non-intervention-annual.geojson?v=20260812-meranti1')]);if(!summaryResponse.ok||!geoResponse.ok)throw new Error('Data analisis awal tidak tersedia');const summary=await summaryResponse.json(),geo=await geoResponse.json();state.preliminarySummary=summary.villages||[];state.preliminaryLayer=L.geoJSON(geo,{style:f=>{const erosion=f.properties.change==='erosion';return {color:erosion?'#c56b17':'#276fbf',weight:2,dashArray:'6 5',fillColor:erosion?'#f0a24a':'#68a8dc',fillOpacity:.24}},onEachFeature:(f,layer)=>{const key=String(f.properties.village||'').toLowerCase();if(!state.preliminaryVillageLayers.has(key))state.preliminaryVillageLayers.set(key,L.featureGroup());state.preliminaryVillageLayers.get(key).addLayer(layer);layer.bindPopup(`<strong>Analisis awal: ${f.properties.change==='erosion'?'indikasi kehilangan':'indikasi pertambahan'} daratan</strong><br>${f.properties.village} · Kabupaten ${f.properties.regency}<br>2016–2025 · keyakinan ${f.properties.confidence}<br><small>Hasil awal desa non-intervensi. Belum terverifikasi lapangan dan bukan penetapan abrasi.</small>`)}}).addTo(map);const analysed=state.preliminarySummary.filter(row=>row.status==='analysed').length,regencies=uniqueSorted(state.preliminarySummary.map(row=>row.regency)).length;$('preliminary-status').innerHTML=`<strong>${analysed} desa tampil</strong><span>${regencies} kabupaten · garis putus-putus</span>`;populateAnalysisFilters();renderAnalysisExplorer()}catch(e){console.warn(e);$('preliminary-status').textContent='Analisis awal belum dapat dimuat';$('analysis-table-body').innerHTML='<tr><td colspan="7">Data analisis belum dapat dimuat.</td></tr>'}}
  async function init(){renderTabs();$('location-name').textContent=state.selected.name;$('location-meta').textContent='Memuat titik model laut terdekat…';await Promise.all([loadAreas(),loadCoastalChange(),loadRehabilitation()]);try{const results=await Promise.allSettled(LOCATIONS.map(fetchMarine));results.forEach((r,i)=>{if(r.status==='fulfilled')state.data.set(LOCATIONS[i].id,r.value)});if(!state.data.size)throw new Error('Data tidak tersedia');$('data-status').textContent='Aktif · 7 desa intervensi YG';$('updated-at').textContent='Cuaca otomatis · abrasi dan rehabilitasi tahunan';renderMarkers();setSelected(state.selected)}catch(e){$('data-status').textContent='7 desa intervensi YG';$('updated-at').textContent='Data laut sedang terganggu · analisis tahunan tetap tersedia';$('map-message').hidden=false;$('map-message').textContent='Data oseanografi belum dapat dimuat. Indikasi perubahan pantai tetap tersedia.';renderMarkers();setSelected(state.selected)}}
  $('location-tabs').addEventListener('click',e=>{const b=e.target.closest('button[data-id]');if(b)setSelected(LOCATIONS.find(l=>l.id===b.dataset.id))});$('toggle-coastal-change').addEventListener('change',e=>{if(!state.changeLayer)return;if(e.target.checked)state.changeLayer.addTo(map);else state.changeLayer.remove()});$('chart-metric').addEventListener('change',()=>{const d=state.data.get(state.selected.id);if(d)renderChart(d)});$('zoom-all').addEventListener('click',()=>map.fitBounds(L.latLngBounds(LOCATIONS.map(l=>[l.lat,l.lon])).pad(.25)));document.querySelectorAll('input[name=product]').forEach(r=>r.addEventListener('change',()=>{state.product=r.value;$('chart-metric').value={wave:'wave_height',current:'ocean_current_velocity',tide:'sea_level_height_msl',sst:'sea_surface_temperature'}[r.value];const d=state.data.get(state.selected.id);if(d){renderDirectionArrows(d,currentHour(d).h);renderChart(d)}}));init();
})();
