(function(){
  ...
  const state={selected:LOCATIONS[0],product:'wave',data:new Map(),markers:new Map(),chart:null,areas:null,waveArrow:null,currentArrow:null,changeLayer:null,changeSummary:new Map(),preliminaryLayer:null,preliminarySummary:[],preliminaryVillageLayers:new Map()};
  ...
  async function init(){renderTabs();$('location-name').textContent=state.selected.name;$('location-meta').textContent='Memuat titik model laut terdekat…';await Promise.all([loadAreas(),loadCoastalChange(),loadPreliminaryCoastalChange()]);try{const results=await Promise.allSettled(LOCATIONS.map(fetchMarine));results.forEach((r,i)=>{if(r.status==='fulfilled')state.data.set(LOCATIONS[i].id,r.value)});if(!state.data.size)throw new Error('Data tidak tersedia');$('data-status').textContent='Aktif · 3 wilayah prioritas mangrove';$('updated-at').textContent='92 desa/kelurahan terbit · abrasi tahunan · cuaca otomatis';renderMarkers();setSelected(state.selected)}catch(e){$('data-status').textContent='3 wilayah prioritas tersedia';$('updated-at').textContent='92 desa/kelurahan terbit · data laut sedang terganggu';$('map-message').hidden=false;$('map-message').textContent='Data oseanografi belum dapat dimuat. Indikasi perubahan pantai tetap tersedia.';renderMarkers();setSelected(state.selected)}}
  $('analysis-regency').addEventListener('change',()=>{populateAnalysisFilters('regency');renderAnalysisExplorer();renderVillageAnalysis(null)});$('analysis-district').addEventListener('change',()=>{populateAnalysisFilters('district');renderAnalysisExplorer();renderVillageAnalysis(null)});$('analysis-village').addEventListener('change',()=>{renderAnalysisExplorer();const row=filteredPreliminaryRows()[0];renderVillageAnalysis(row||null);if(row)focusPreliminaryVillage(row.village)});$('analysis-table-body').addEventListener('click',e=>{const trigger=e.target.closest('[data-detail-village],tr[data-village-row]');if(!trigger)return;const name=trigger.dataset.detailVillage||trigger.dataset.villageRow,row=state.preliminarySummary.find(item=>item.village===name);if(row){renderVillageAnalysis(row);focusPreliminaryVillage(row.village)}});$('analysis-village-detail').addEventListener('click',e=>{const button=e.target.closest('[data-focus-village]');if(button)focusPreliminaryVillage(button.dataset.focusVillage)});
