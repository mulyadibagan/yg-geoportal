(function(){
  ...
  function renderRehabilitation(){const row=state.rehabilitationSummary.get(state.selected.name.toLowerCase()),box=$('rehabilitation-summary');if(!row){box.innerHTML='<strong>Prioritas rehabilitasi mangrove</strong><p>Hasil penilaian belum tersedia untuk lokasi ini.</p>';return}box.innerHTML=`<strong>Prioritas rehabilitasi mangrove</strong><p>${row.recommendedAction||'Tinjau kandidat rehabilitasi pada peta prioritas.'}</p><div class="rehab-metrics"><span>Luas prioritas<b>${fmt(row.priorityAreaHa||row.candidateAreaHa,2)} ha</b></span><span>Polygon kandidat<b>${row.priorityPolygonCount||0}</b></span><span>Kebutuhan<b>${row.needScore||0}/100 · ${row.needClass||'—'}</b></span><span>Kelayakan<b>${row.suitabilityScore||0}/100 · ${row.suitabilityClass||'—'}</b></span></div><a href="mangrove-priority.html?regency=${state.selected.regency.toLowerCase()}&village=${state.selected.id}">Buka polygon prioritas →</a>`}
