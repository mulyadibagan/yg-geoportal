(function(){
  ...
  var today=new Date(),dateInput=document.getElementById('observation-date');today.setDate(today.getDate()-1);var observationDate=today.toISOString().slice(0,10);dateInput.value=observationDate;dateInput.max=observationDate;
  ...
  function selectProduct(id){document.querySelectorAll('[data-product]').forEach(function(b){b.classList.toggle('active',b.dataset.product===id)});if(id==='hotspots'){setLayerChecked('hotspots',true)}if(id==='wind'){setLayerChecked('wind',true)}if(id==='rain'){setLayerChecked('rain',true)}if(id==='satellite'){setLayerChecked('satellite',true);setLayerChecked('haze',true)}}
  function updateObservationDate(value){observationDate=value;nationalHotspots.setParams({time:value});hazeLayer.setParams({time:value});satelliteLayer.setParams({time:value});document.getElementById('condition-title').textContent='Pengamatan satelit '+new Date(value+'T12:00:00').toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'});document.getElementById('condition-copy').textContent='Pilih produk untuk membandingkan hotspot, arah angin, akumulasi hujan terbaru, serta indikasi haze/AOD.'}
  function addMapBadge(){var badge=L.control({position:'bottomleft'});badge.onAdd=function(){var div=L.DomUtil.create('div','fw-layer-badge');div.innerHTML='<strong>CAKUPAN INDONESIA</strong><span>Hotspot VIIRS + indikasi kabut asap MODIS AOD</span>';return div};badge.addTo(map)}
  ...
  dateInput.addEventListener('change',function(){if(dateInput.value)updateObservationDate(dateInput.value)});updateObservationDate(observationDate);
