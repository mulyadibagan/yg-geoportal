(function(){
  ...
  var readingGuide=document.querySelector('.fw-disclaimer p');if(readingGuide)readingGuide.textContent='Polygon menunjukkan koridor kumulatif potensi transport udara sejak deteksi hotspot pada tiga lapisan GFS. Zona utama, batas ketidakpastian, dan area yang saling menumpuk bukan plume asap teramati, bukan konsentrasi PM2.5, dan bukan prakiraan kesehatan.';
  ...
  function renderSourceEnvelope(source,trajectories,en){
    var factor=smokeModel.DEFAULT_ENVELOPE.outerRadiusFactor,core=mergeEnvelopeFeatures(trajectories.map(function(trajectory){return smokeModel.buildEnvelopePolygon(trajectory,{band:'core',radiusFactor:1})})),outer=mergeEnvelopeFeatures(trajectories.map(function(trajectory){return smokeModel.buildEnvelopePolygon(trajectory,{band:'outer',radiusFactor:factor})})),html=envelopePopup(source,trajectories,en),outerLayer=null,coreLayer=null,sourceBounds=L.latLngBounds([]);
    if(outer){outerLayer=L.geoJSON(outer,{pane:'smokePane',style:{color:'#766b63',weight:2,dashArray:'7 5',fillColor:'#a69b92',fillOpacity:.18}}).bindPopup(html,{maxWidth:380}).bindTooltip(en?'Model uncertainty boundary':'Batas ketidakpastian model').addTo(groups.smoke);sourceBounds.extend(outerLayer.getBounds())}
    if(core){coreLayer=L.geoJSON(core,{pane:'smokePane',style:{color:'#4f4742',weight:1.5,fillColor:'#6f655e',fillOpacity:.34}}).bindPopup(html,{maxWidth:380}).bindTooltip(en?'Primary model zone':'Zona utama model').addTo(groups.smoke);sourceBounds.extend(coreLayer.getBounds())}
    var sourceTime=smokeModel.featureTime(source);if(sourceBounds.isValid()&&sourceTime>=smokeFocusTime){smokeBounds=sourceBounds;smokeFocusTime=sourceTime}
  ...
  function renderSmoke(){
    ...
    smokeBounds=L.latLngBounds([]);smokeFocusTime=-Infinity;
  ...
  function selectProduct(id){document.querySelectorAll('[data-product]').forEach(function(b){b.classList.toggle('active',b.dataset.product===id)});if(id==='hotspots'){setLayerChecked('hotspots',true)}if(id==='wind'){setLayerChecked('wind',true)}if(id==='rain'){setLayerChecked('rain',true)}if(id==='smoke'){smokeAutoFit=true;if(period===7||period===30)applyPeriodButton(document.querySelector('#period-control [data-period="latest"]'));setLayerChecked('smoke',true);setLayerChecked('hotspots',false);setLayerChecked('wind',true)}}
