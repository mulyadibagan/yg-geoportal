(function(){
  ...
  map.createPane('smokePane');map.getPane('smokePane').style.zIndex=410;
  ...
  var readingGuide=document.querySelector('.fw-disclaimer p');if(readingGuide)readingGuide.textContent='Kontur menunjukkan dukungan relatif lintasan transport pada jendela waktu aktif. Hijau hingga merah berarti dukungan model makin kuat dalam pemodelan yang sama, bukan konsentrasi asap, PM2.5, probabilitas, atau tingkat bahaya kesehatan.';
  ...
  function updateSmokeProductLabel(){var en=smokeEnglish(),title=document.getElementById('smoke-product-title'),note=document.getElementById('smoke-product-note');if(period===7){title.textContent=en?'Repeated Fire Sources':'Sumber Api Berulang';note.textContent=en?'At least 2 days within 7 days':'Minimal 2 hari dalam 7 hari'}else if(period===30){title.textContent=en?'Contours Disabled':'Kontur Dinonaktifkan';note.textContent=en?'Historical detections only':'Hanya deteksi historis'}else{title.textContent=en?'Experimental Smoke-Transport Contours':'Kontur Transport Asap Eksperimental';note.textContent=period==='latest'?(en?'Relative model support · 6-hour sources':'Dukungan model relatif · sumber 6 jam'):(en?'Relative model support · 24-hour sources':'Dukungan model relatif · sumber 24 jam')}}
  var smokeContourStyles=[
    {id:'outer',percentile:20,labelId:'Jejak model terluar',labelEn:'Outer model trace',color:'#4c9d68',fill:'#8ccf8a',opacity:.18},
    {id:'limited',percentile:50,labelId:'Dukungan model terbatas',labelEn:'Limited model support',color:'#d0aa24',fill:'#f0d15c',opacity:.22},
    {id:'moderate',percentile:75,labelId:'Dukungan model sedang',labelEn:'Moderate model support',color:'#e57e1f',fill:'#ee9d43',opacity:.26},
    {id:'strong',percentile:90,labelId:'Dukungan model kuat',labelEn:'Strong model support',color:'#cf3b32',fill:'#e65b4f',opacity:.31}
  ];
  function contourPopup(style,field,meta,en){
    var label=en?style.labelEn:style.labelId,threshold=Math.round(style.percentile),sources=meta.sources,detections=meta.detections,paths=meta.trajectories;
    return en?'<strong>'+esc(label)+'</strong><br>Relative class starts at P'+threshold+' of positive model-grid cells.<br>Source clusters: '+sources+' ('+detections+' raw detections)<br>Height-sensitive paths: '+paths+'<br>Kernel bandwidth: about '+Math.round(field.bandwidthKm)+' km<br>Contour grid: '+field.gridStepDegrees.toFixed(2)+'°<br><hr><small>Each clustered source has equal total weight. FRP and AOD do not set the colors. The Gaussian kernel smooths trajectory support at the sampled wind-grid scale; it is not physical dispersion, concentration, probability, or a health category.</small>':'<strong>'+esc(label)+'</strong><br>Kelas relatif dimulai pada P'+threshold+' dari sel grid model bernilai positif.<br>Kelompok sumber: '+sources+' ('+detections+' deteksi mentah)<br>Lintasan sensitif-ketinggian: '+paths+'<br>Lebar kernel: sekitar '+Math.round(field.bandwidthKm)+' km<br>Grid kontur: '+field.gridStepDegrees.toFixed(2)+'°<br><hr><small>Setiap kelompok sumber memiliki bobot total yang sama. FRP dan AOD tidak menentukan warna. Kernel Gaussian hanya menghaluskan dukungan lintasan pada skala sampling grid angin; ini bukan dispersi fisik, konsentrasi, probabilitas, atau kategori kesehatan.</small>';
  }
  function renderContourBands(field,meta,en){
    var properties=smokeContourStyles.map(function(style,index){return {contour_index:index,contour_id:style.id,percentile:style.percentile}}),bands;
    try{bands=turf.isobands(field.grid,field.breaks,{zProperty:'support',breaksProperties:properties})}catch(error){return {bandCount:0,polygonCount:0,error:error}}
    ...
    (bands.features||[]).forEach(function(feature,index){
      ...
      try{turf.flattenEach(feature,function(){polygonCount+=1})}catch(error){polygonCount+=1}
    });
    return {bandCount:bandCount,polygonCount:polygonCount,error:null};
  }
  function renderSmoke(){
    groups.smoke.clearLayers();
    smokeBounds=L.latLngBounds([]);
  ...
  function applyPeriodButton(button){if(!button)return;period=button.dataset.period==='latest'?'latest':Number(button.dataset.period);document.querySelectorAll('#period-control button').forEach(function(item){item.classList.toggle('active',item===button)});document.getElementById('kpi-period').textContent=button.textContent;refreshHotspots();renderSmoke();requestTransportIfNeeded()}
  function selectProduct(id){document.querySelectorAll('[data-product]').forEach(function(b){b.classList.toggle('active',b.dataset.product===id)});if(id==='hotspots'){setLayerChecked('hotspots',true)}if(id==='wind'){setLayerChecked('wind',true)}if(id==='rain'){setLayerChecked('rain',true)}if(id==='smoke'){smokeAutoFit=true;if(period===7||period===30)applyPeriodButton(document.querySelector('#period-control [data-period="1"]'));setLayerChecked('smoke',true);setLayerChecked('hotspots',true);setLayerChecked('wind',true)}}
  ...
  document.getElementById('period-control').addEventListener('click',function(e){var b=e.target.closest('button');if(b)applyPeriodButton(b)});
