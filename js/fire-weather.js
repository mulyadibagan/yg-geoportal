(function(){
  ...
  function smokeContours(field){
    ...
    var parsed=keys.map(function(key){return key.split('|').map(Number)}),ys=parsed.map(function(p){return p[0]}),xs=parsed.map(function(p){return p[1]}),minY=Math.min.apply(null,ys)-2,maxY=Math.max.apply(null,ys)+2,minX=Math.min.apply(null,xs)-2,maxX=Math.max.apply(null,xs)+2,maxDensity=Math.max.apply(null,keys.map(function(key){return field.cells[key]})),points=[];
    for(var yi=minY;yi<=maxY;yi++)for(var xi=minX;xi<=maxX;xi++){var lat=yi*field.step-12,lon=xi*field.step+94,density=field.cells[yi+'|'+xi]||0,relative=density/(maxDensity||1),aerosol=nearestAerosol(lat,lon),score=relative<.025?0:Math.min(100,Math.round(relative*75+aerosolPoints(aerosol&&aerosol.aod||0)));points.push(turf.point([lon,lat],{score:score}))}
  ...
  function renderSmoke(){
    ...
    results.forEach(function(result){var html=en?'<strong>Connected smoke-potential contour — '+smokeLabel(result.score)+'</strong><br>Score band: '+result.score+'/100<br>Trajectory horizon: 24 hours<br>Maximum simulated displacement: '+Math.round(field.maxTravel)+' km<br><hr><small>Hourly particle advection using 925 hPa wind, rain attenuation, FIRMS FRP, and CAMS AOD. Indicative model, not observed smoke.</small>':'<strong>Kontur terkoneksi potensi asap — '+smokeLabel(result.score)+'</strong><br>Kelas skor: '+result.score+'/100<br>Horizon lintasan: 24 jam<br>Perpindahan simulasi maksimum: '+Math.round(field.maxTravel)+' km<br><hr><small>Adveksi partikel per jam menggunakan angin 925 hPa, reduksi hujan, FRP FIRMS, dan AOD CAMS. Model indikatif, bukan asap teramati.</small>';L.geoJSON(result.feature,{pane:'smokePane',style:{color:smokeColor(result.score),weight:2,fillColor:smokeColor(result.score),fillOpacity:.3}}).bindPopup(html,{maxWidth:330}).addTo(groups.smoke)});
    ...
    summary.innerHTML=en?'<strong>Highest potential: '+smokeLabel(top.score)+' ('+top.score+'/100)</strong><p>'+results.length+' connected contours from '+items.length+' hotspots. 925 hPa trajectory: '+esc(transportTime||'latest')+' WIB · CAMS AOD: '+esc(aerosolTime||'latest')+' WIB.</p>':'<strong>Potensi tertinggi: '+smokeLabel(top.score)+' ('+top.score+'/100)</strong><p>'+results.length+' kontur terkoneksi dari '+items.length+' hotspot. Lintasan 925 hPa: '+esc(transportTime||'terbaru')+' WIB · AOD CAMS: '+esc(aerosolTime||'terbaru')+' WIB.</p>';
  }
  ...
  function loadTransportWeather(){
    ...
    for(var lat=-10;lat<=5;lat+=3)for(var lon=96;lon<=140;lon+=4)grid.push([lat,lon]);
    var lats=grid.map(function(p){return p[0]}).join(','),lons=grid.map(function(p){return p[1]}).join(',');
    var url='https://api.open-meteo.com/v1/forecast?latitude='+lats+'&longitude='+lons+'&hourly=wind_speed_925hPa,wind_direction_925hPa,precipitation&past_days=1&forecast_days=1&timezone=Asia%2FJakarta';
