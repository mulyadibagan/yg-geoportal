(function(){
  ...
  var readingGuide=document.querySelector('.fw-disclaimer p');if(readingGuide)readingGuide.textContent='Garis menunjukkan rekonstruksi arah perjalanan udara sejak waktu deteksi hotspot hingga waktu model sekarang pada tiga lapisan GFS. Garis ini bukan plume asap teramati, bukan konsentrasi PM2.5, dan bukan prakiraan kesehatan.';
  ...
  function updateSmokeProductLabel(){var en=smokeEnglish(),title=document.getElementById('smoke-product-title'),note=document.getElementById('smoke-product-note');if(period===7){title.textContent=en?'Repeated Fire Sources':'Sumber Api Berulang';note.textContent=en?'At least 2 days within 7 days':'Minimal 2 hari dalam 7 hari'}else if(period===30){title.textContent=en?'Trajectory Disabled':'Jejak Dinonaktifkan';note.textContent=en?'Historical detections only':'Hanya deteksi historis'}else{title.textContent=en?'Experimental Smoke Transport':'Jejak Transport Asap Eksperimental';note.textContent=period==='latest'?(en?'Sources detected in the last 6 hours':'Sumber terdeteksi 6 jam terakhir'):(en?'Sources detected in the last 24 hours':'Sumber terdeteksi 24 jam terakhir')}}
  ...
  function renderTrajectory(trajectory,en,sourceMarkers){
    var properties=trajectory.source.properties||{},sourceCount=Number(properties.source_count)||1,satellites=(properties.satellites||[]).join(', ')||properties.satellite||'—',detected=new Date(trajectory.startTime).toLocaleString(en?'en-US':'id-ID',{timeZone:'Asia/Jakarta',dateStyle:'medium',timeStyle:'short'})+' WIB',linePoints=trajectory.path.map(function(c){return [c[1],c[0]]}),level=trajectory.level;
    var html=en?'<strong>Experimental transport track · '+levelName(level,true)+'</strong><br>Source observation cluster: '+sourceCount+' detection(s)<br>Satellite(s): '+esc(satellites)+'<br>Track starts: '+esc(detected)+'<br>Simulated duration: '+trajectory.durationHours.toFixed(1)+' hours<br>Horizontal displacement: '+Math.round(trajectory.travelKm)+' km<br><hr><small>Horizontal advection using space-time interpolated GFS wind. This is not an observed plume, a concentration estimate, or a health forecast.</small>':'<strong>Jejak transport eksperimental · '+levelName(level,false)+'</strong><br>Kelompok observasi sumber: '+sourceCount+' deteksi<br>Satelit: '+esc(satellites)+'<br>Jejak dimulai: '+esc(detected)+'<br>Durasi simulasi: '+trajectory.durationHours.toFixed(1)+' jam<br>Perpindahan horizontal: '+Math.round(trajectory.travelKm)+' km<br><hr><small>Adveksi horizontal memakai angin GFS yang diinterpolasi ruang–waktu. Ini bukan plume teramati, perkiraan konsentrasi, atau prakiraan kesehatan.</small>';
    L.polyline(linePoints,{pane:'smokePane',color:level.color,weight:3,opacity:.82,dashArray:level.dashArray||null,lineCap:'round'}).bindPopup(html,{maxWidth:350}).addTo(groups.smoke);
    var last=linePoints[linePoints.length-1];L.circleMarker(last,{pane:'smokePane',radius:3,color:level.color,weight:2,fillColor:'#fff',fillOpacity:1}).bindTooltip(level.pressure+' hPa').addTo(groups.smoke);
    if(!sourceMarkers[trajectory.sourceIndex]){sourceMarkers[trajectory.sourceIndex]=true;L.circleMarker(linePoints[0],{pane:'smokePane',radius:5,color:'#5a2811',weight:2,fillColor:'#fff',fillOpacity:1}).bindPopup(html,{maxWidth:350}).addTo(groups.smoke)}
  }
  function renderSmoke(){
    groups.smoke.clearLayers();
    var summary=document.getElementById('smoke-summary'),en=smokeEnglish();
    updateSmokeProductLabel();
    if(period===30){summary.className='fw-smoke-summary';summary.innerHTML=en?'<strong>30-day hotspot history</strong><p>Transport tracks are disabled. This period is used only for historical hotspot detections and statistics.</p>':'<strong>Riwayat hotspot 30 hari</strong><p>Jejak transport dinonaktifkan. Periode ini hanya untuk deteksi dan statistik hotspot historis.</p>';return}
    ...
    if(!hotspotGeo||!transportReady||!smokeModel||!windIndex){
      summary.className='fw-smoke-summary';
      summary.innerHTML=en?'<strong>Experimental smoke transport</strong><p>Cannot be reconstructed: hotspot or multi-level GFS wind data are incomplete.</p>':'<strong>Jejak transport asap eksperimental</strong><p>Belum dapat direkonstruksi: data hotspot atau angin GFS multi-lapisan belum lengkap.</p>';
      return;
    }
    if(observationDate!==currentDate){summary.className='fw-smoke-summary';summary.innerHTML=en?'<strong>Experimental smoke transport</strong><p>The reconstruction is available only for the current date because historical multi-level wind has not been loaded.</p>':'<strong>Jejak transport asap eksperimental</strong><p>Rekonstruksi hanya tersedia untuk tanggal hari ini karena angin historis multi-lapisan belum dimuat.</p>';return}
    ...
    var sources=smokeModel.clusterSources(detections,{spatialKm:1.5,temporalMinutes:90}),trajectories=smokeModel.buildTrajectories(sources,windIndex,periodEnd().getTime(),{maxHours:modelHours}),sourceMarkers={};
    trajectories.forEach(function(trajectory){renderTrajectory(trajectory,en,sourceMarkers)});
    if(!trajectories.length){summary.className='fw-smoke-summary';summary.innerHTML=en?'<strong>Experimental smoke transport</strong><p>No track could be reconstructed within the available GFS grid and time window.</p>':'<strong>Jejak transport asap eksperimental</strong><p>Tidak ada jejak yang dapat direkonstruksi dalam cakupan grid dan waktu GFS yang tersedia.</p>';return}
    summary.className='fw-smoke-summary';
    summary.innerHTML=en?'<strong>Experimental transport · '+modelHours+'-hour source window</strong><p>'+trajectories.length+' tracks at three pressure levels from '+sources.length+' observation clusters ('+detections.length+' raw detections). GFS grid sampled every 1.5° and interpolated in space and time; data coverage '+transportCoverage+'%. Model time: '+esc(transportTime||'latest')+' WIB.</p>':'<strong>Transport eksperimental · jendela sumber '+modelHours+' jam</strong><p>'+trajectories.length+' jejak pada tiga lapisan dari '+sources.length+' kelompok observasi ('+detections.length+' deteksi mentah). Grid GFS disampel setiap 1,5° dan diinterpolasi ruang–waktu; cakupan data '+transportCoverage+'%. Waktu model: '+esc(transportTime||'terbaru')+' WIB.</p>';
  }
