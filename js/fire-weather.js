(function(){
  ...
  function renderSourceEnvelope(source,trajectories,en){
    var factor=smokeModel.DEFAULT_ENVELOPE.outerRadiusFactor,core=mergeEnvelopeFeatures(trajectories.map(function(trajectory){return smokeModel.buildEnvelopePolygon(trajectory,{band:'core',radiusFactor:1})})),outer=mergeEnvelopeFeatures(trajectories.map(function(trajectory){return smokeModel.buildEnvelopePolygon(trajectory,{band:'outer',radiusFactor:factor})})),html=envelopePopup(source,trajectories,en),outerLayer=null,coreLayer=null;
    if(outer){outerLayer=L.geoJSON(outer,{pane:'smokePane',style:{color:'#766b63',weight:2,dashArray:'7 5',fillColor:'#a69b92',fillOpacity:.15}}).bindPopup(html,{maxWidth:380}).bindTooltip(en?'Model uncertainty boundary':'Batas ketidakpastian model').addTo(groups.smoke);if(smokeBounds)smokeBounds.extend(outerLayer.getBounds())}
    if(core){coreLayer=L.geoJSON(core,{pane:'smokePane',style:{color:'#4f4742',weight:1.5,fillColor:'#6f655e',fillOpacity:.28}}).bindPopup(html,{maxWidth:380}).bindTooltip(en?'Primary model zone':'Zona utama model').addTo(groups.smoke);if(smokeBounds)smokeBounds.extend(coreLayer.getBounds())}
