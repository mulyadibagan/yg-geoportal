(function(){
  ...
  var villageGeo=null,analytics=null,hotspotGeo=null,ygBounds=null,period=30,rainLayer=null,mapDateBadge=null,hotspotStatusText='Memuat…',weatherReadings=[],weatherReady=false,aerosolReadings=[],aerosolReady=false,aerosolTime='';
  ...
  function smokePolygon(cluster,weather,aerosol){var travel=(Number(weather.direction)||0)+180,speed=Math.max(0,Number(weather.speed)||0),rain=Math.max(0,Number(weather.rain)||0),aod=Math.max(0,Number(aerosol&&aerosol.aod)||0),fire=Math.min(35,Math.round(cluster.count*6+Math.log1p(cluster.frp)*5)),transport=Math.min(25,Math.round(4+speed)),aerosolScore=aerosolPoints(aod),rainScore=rain===0?15:rain<.5?10:rain<2?4:0,score=Math.min(100,fire+transport+aerosolScore+rainScore),length=Math.max(35,Math.min(180,25+speed*5+Math.sqrt(cluster.frp)*3)),origin=[cluster.lat,cluster.lon],shape=[origin,destination(cluster.lat,cluster.lon,travel-30,length*.28),destination(cluster.lat,cluster.lon,travel-18,length),destination(cluster.lat,cluster.lon,travel,length*1.08),destination(cluster.lat,cluster.lon,travel+18,length),destination(cluster.lat,cluster.lon,travel+30,length*.28)];return {shape:shape,score:score,label:smokeLabel(score),className:smokeClass(score),color:smokeColor(score),direction:travel%360,length:length,fire:fire,transport:transport,aerosolScore:aerosolScore,rainScore:rainScore,weather:weather,aerosol:aerosol,cluster:cluster}}
  function renderSmoke(){
    groups.smoke.clearLayers();
    var summary=document.getElementById('smoke-summary'),en=smokeEnglish();
    if(!hotspotGeo||!weatherReady||!aerosolReady){
      summary.className='fw-smoke-summary';
      summary.innerHTML=en?'<strong>Smoke dispersion potential</strong><p>Cannot be assessed: hotspot, weather, or aerosol data are incomplete.</p>':'<strong>Potensi sebaran asap</strong><p>Belum dapat dinilai: data hotspot, cuaca, atau aerosol belum lengkap.</p>';
      return;
    }
    ...
    var results=smokeClusters(items).map(function(cluster){return smokePolygon(cluster,nearestWeather(cluster.lat,cluster.lon),nearestAerosol(cluster.lat,cluster.lon))});
    results.forEach(function(result){
      var w=result.weather,a=result.aerosol,c=result.cluster;
      var html=en?'<strong>Smoke dispersion potential — '+result.label+' ('+result.score+'/100)</strong><br>Indicative zone: ±'+Math.round(result.length)+' km toward '+Math.round(result.direction)+'°<br>Source: '+c.count+' hotspots · total FRP '+c.frp.toFixed(1)+' MW<br>Wind: '+Math.round(w.speed)+' km/h · rain '+w.rain+' mm<br>AOD 550 nm: '+a.aod.toFixed(2)+' · PM2.5 model: '+a.pm25.toFixed(1)+' µg/m³<br><hr><small>Heat source '+result.fire+'/35 · wind '+result.transport+'/25 · aerosol '+result.aerosolScore+'/25 · rain '+result.rainScore+'/15.<br>CAMS model aerosol, not an observed smoke boundary or a ground sensor reading.</small>':'<strong>Potensi sebaran asap — '+result.label+' ('+result.score+'/100)</strong><br>Zona indikatif: ±'+Math.round(result.length)+' km ke '+Math.round(result.direction)+'°<br>Sumber: '+c.count+' hotspot · total FRP '+c.frp.toFixed(1)+' MW<br>Angin: '+Math.round(w.speed)+' km/j · hujan '+w.rain+' mm<br>AOD 550 nm: '+a.aod.toFixed(2)+' · PM2.5 model: '+a.pm25.toFixed(1)+' µg/m³<br><hr><small>Sumber panas '+result.fire+'/35 · angin '+result.transport+'/25 · aerosol '+result.aerosolScore+'/25 · hujan '+result.rainScore+'/15.<br>Aerosol model CAMS, bukan batas asap teramati atau pembacaan sensor darat.</small>';
    ...
    summary.innerHTML=en?'<strong>Highest potential: '+top.label+' ('+top.score+'/100)</strong><p>'+results.length+' indicative zones from '+items.length+' hotspots in 24 hours. CAMS AOD: '+esc(aerosolTime||'latest available')+' WIB. Click a polygon for the score breakdown.</p>':'<strong>Potensi tertinggi: '+top.label+' ('+top.score+'/100)</strong><p>'+results.length+' zona indikatif dari '+items.length+' hotspot dalam 24 jam. AOD CAMS: '+esc(aerosolTime||'waktu terbaru tersedia')+' WIB. Klik poligon untuk rincian skor.</p>';
  }
