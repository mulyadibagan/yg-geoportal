(function(){
  ...
  var readingGuide=document.querySelector('.fw-disclaimer p');if(readingGuide)readingGuide.textContent='Titik merah tua adalah sumber kompleks kebakaran; garis putus-putus menunjukkan lintasan median ensemble menuju kontur peluang transport. Kontur bukan batas asap teramati dan bukan pengukuran kualitas udara.';
  ...
  function nearestTransport(lat,lon,timeMs){var row=transportReadings.reduce(function(best,item){var d=distanceKm([lat,lon],[item.lat,item.lon]);return !best||d<best.distance?Object.assign({distance:d},item):best},null);if(!row)return null;var index=0,bestTime=Infinity;row.times.forEach(function(t,i){var delta=Math.abs(t-timeMs);if(delta<bestTime){bestTime=delta;index=i}});return {speed:Number(row.speeds[index])||0,direction:Number(row.directions[index])||0,rain:Number(row.rain[index])||0,vertical:Number(row.vertical[index])||0,pbl:Number(row.pbl[index])||0,time:row.times[index]}}
  function fireComplexes(items){var buckets={};items.forEach(function(feature){var c=feature.geometry.coordinates,p=feature.properties||{},key=Math.round(c[1]*4)+'|'+Math.round(c[0]*4),time=pointTime(feature).getTime();if(!buckets[key])buckets[key]={lat:0,lon:0,count:0,frp:0,time:0};var b=buckets[key];b.lat+=c[1];b.lon+=c[0];b.count++;b.frp+=Math.max(0,Number(p.frp)||0);b.time=Math.max(b.time,time)});return Object.keys(buckets).map(function(key){var b=buckets[key];b.lat/=b.count;b.lon/=b.count;return b})}
  function nearestEnsemble(lat,lon,timeMs){var row=ensembleReadings.reduce(function(best,item){var d=distanceKm([lat,lon],[item.lat,item.lon]);return !best||d<best.distance?Object.assign({distance:d},item):best},null);if(!row)return [];var index=0,bestTime=Infinity;row.times.forEach(function(t,i){var delta=Math.abs(t-timeMs);if(delta<bestTime){bestTime=delta;index=i}});return row.members.map(function(member){return {speed:Number(member.speeds[index])||0,direction:Number(member.directions[index])||0}}).filter(function(w){return w.speed>0})}
  function smokeField(items){
    var step=.25,cells={},end=periodEnd().getTime(),maxTravel=0,complexes=fireComplexes(items),traces=[];
    ...
    complexes.forEach(function(source){var time=Math.max(source.time,end-86400000),members=nearestEnsemble(source.lat,source.lon,time),tracks=members.length?[members[0],members[Math.floor(members.length*.25)],members[Math.floor(members.length*.5)],members[Math.floor(members.length*.75)],members[members.length-1]]:[null];tracks.forEach(function(seed,trackIndex){var lat=source.lat,lon=source.lon,origin=[lat,lon],path=[[lat,lon]],trackTime=time,weight=(1+Math.sqrt(source.frp)/3+Math.log1p(source.count))/tracks.length;for(var hour=0;hour<24&&trackTime<=end;hour++){var wind=nearestTransport(lat,lon,trackTime);if(!wind)break;var currentMembers=nearestEnsemble(lat,lon,trackTime),member=seed&&currentMembers.length?currentMembers[Math.min(currentMembers.length-1,members.indexOf(seed))]:null,direction=member?member.direction:wind.direction,speed=member?member.speed:wind.speed,pbl=Math.max(100,wind.pbl||800),verticalPenalty=Math.abs(wind.vertical)>.2?.82:1,mixing=Math.max(.65,Math.min(1.2,pbl/900))*verticalPenalty;deposit(lat,lon,weight*mixing);var next=destination(lat,lon,(direction+180)%360,speed),rainLoss=wind.rain>=2?.42:wind.rain>=.5?.65:.92;weight*=rainLoss;if(weight<.05)break;lat=next[0];lon=next[1];path.push([lat,lon]);trackTime+=3600000;maxTravel=Math.max(maxTravel,distanceKm(origin,[lat,lon]))}if(trackIndex===Math.floor(tracks.length/2)&&path.length>1)traces.push({path:path,source:source})})});
    return {cells:cells,step:step,maxTravel:maxTravel,complexCount:complexes.length,complexes:complexes,traces:traces,ensemble:ensembleReady};
  }
  ...
  function renderSmoke(){
    ...
    if(!hotspotGeo||!transportReady||!aerosolReady||!window.turf){
      ...
      summary.innerHTML=en?'<strong>Smoke transport likelihood</strong><p>Cannot be assessed: hotspot, GFS, GEFS, aerosol, or contour data are incomplete.</p>':'<strong>Peluang transport asap</strong><p>Belum dapat dinilai: data hotspot, GFS, GEFS, aerosol, atau kontur belum lengkap.</p>';
      return;
    }
    ...
    if(!items.length){
      summary.className='fw-smoke-summary';
      summary.innerHTML=en?'<strong>Smoke transport likelihood: low</strong><p>No high-confidence hotspot was found in the '+modelHours+'-hour model. This does not guarantee smoke-free air.</p>':'<strong>Peluang transport asap: rendah</strong><p>Tidak ada hotspot confidence tinggi dalam model '+modelHours+' jam. Ini bukan jaminan udara bebas asap.</p>';
      return;
    }
    ...
    field.complexes.forEach(function(source){var sourceTime=new Date(source.time).toLocaleString(en?'en-GB':'id-ID',{timeZone:'Asia/Jakarta',dateStyle:'medium',timeStyle:'short'}),html=en?'<strong>Fire-complex source</strong><br>'+source.count+' hotspot detections<br>Total FRP: '+source.frp.toFixed(1)+' MW<br>'+sourceTime+' WIB<br><small>Dashed line shows the median ensemble transport path.</small>':'<strong>Sumber kompleks kebakaran</strong><br>'+source.count+' deteksi hotspot<br>Total FRP: '+source.frp.toFixed(1)+' MW<br>'+sourceTime+' WIB<br><small>Garis putus-putus menunjukkan lintasan median ensemble.</small>';L.circleMarker([source.lat,source.lon],{pane:'hotspotPane',className:'smoke-source-marker',radius:8,color:'#fff',weight:2,fillColor:'#7f1d1d',fillOpacity:1}).bindTooltip(en?'Fire source':'Sumber api').bindPopup(html).addTo(groups.smoke)});
    ...
    if(!results.length){summary.className='fw-smoke-summary';summary.innerHTML=en?'<strong>Smoke transport likelihood</strong><p>No connected contour passed the minimum model threshold.</p>':'<strong>Peluang transport asap</strong><p>Tidak ada kontur terkoneksi yang melewati ambang minimum model.</p>';return}
    var top=results.sort(function(a,b){return b.score-a.score})[0];
    summary.className='fw-smoke-summary '+smokeClass(top.score);
    summary.innerHTML=en?'<strong>'+modelHours+'-hour smoke transport likelihood: '+smokeLabel(top.score)+' ('+top.score+'/100)</strong><p>'+results.length+' connected contours from '+field.complexCount+' fire complexes ('+items.length+' hotspots). GFS 925 hPa + PBL/vertical motion/rain; GEFS '+(field.ensemble?'ensemble active':'fallback unavailable')+'.</p>':'<strong>Peluang transport asap '+modelHours+' jam: '+smokeLabel(top.score)+' ('+top.score+'/100)</strong><p>'+results.length+' kontur terkoneksi dari '+field.complexCount+' kompleks kebakaran ('+items.length+' hotspot). GFS 925 hPa + PBL/gerak vertikal/hujan; GEFS '+(field.ensemble?'ensemble aktif':'fallback tidak tersedia')+'.</p>';
    summary.insertAdjacentHTML('beforeend','<p id="smoke-source-visibility"></p>');updateSourceVisibility();
  }
  function updateSourceVisibility(){var note=document.getElementById('smoke-source-visibility');if(!note||!lastSmokeField)return;var en=smokeEnglish(),visible=lastSmokeField.complexes.filter(function(source){return map.getBounds().contains([source.lat,source.lon])}).length;note.textContent=visible?(en?visible+' fire-complex source(s) are inside the current map view.':visible+' sumber kompleks kebakaran berada di dalam tampilan peta.'):(en?'No source is inside the current map view; the contour is a transported path from an off-screen source.':'Tidak ada sumber di dalam tampilan peta; kontur merupakan lintasan transport dari sumber di luar layar.')}
  ...
  function loadTransportWeather(){
    ...
    var lats=grid.map(function(p){return p[0]}).join(','),lons=grid.map(function(p){return p[1]}).join(',');
    var url='https://api.open-meteo.com/v1/gfs?latitude='+lats+'&longitude='+lons+'&hourly=wind_speed_925hPa,wind_direction_925hPa,vertical_velocity_925hPa,boundary_layer_height,precipitation&past_days=1&forecast_days=1&timezone=Asia%2FJakarta';
    return fetch(url).then(function(r){if(!r.ok)throw Error('925 hPa wind');return r.json()}).then(function(data){
      var rows=Array.isArray(data)?data:[data];
      transportReadings=rows.map(function(row,i){var h=row.hourly||{},point=grid[i]||grid[0];return {lat:Number(row.latitude)||point[0],lon:Number(row.longitude)||point[1],times:(h.time||[]).map(function(t){return new Date(t+'+07:00').getTime()}),speeds:h.wind_speed_925hPa||[],directions:h.wind_direction_925hPa||[],vertical:h.vertical_velocity_925hPa||[],pbl:h.boundary_layer_height||[],rain:h.precipitation||[]}});
  ...
  function loadEnsembleWeather(){
    ...
    var lats=grid.map(function(p){return p[0]}).join(','),lons=grid.map(function(p){return p[1]}).join(','),url='https://ensemble-api.open-meteo.com/v1/ensemble?latitude='+lats+'&longitude='+lons+'&hourly=wind_speed_925hPa,wind_direction_925hPa&models=gfs_seamless&forecast_days=1&timezone=Asia%2FJakarta';
    return fetch(url).then(function(r){if(!r.ok)throw Error('GEFS');return r.json()}).then(function(data){var rows=Array.isArray(data)?data:[data];ensembleReadings=rows.map(function(row,i){var h=row.hourly||{},point=grid[i]||grid[0],keys=Object.keys(h),speedKeys=keys.filter(function(k){return /^wind_speed_925hPa(_member\d+)?$/.test(k)}),members=speedKeys.map(function(speedKey){var suffix=speedKey.replace('wind_speed_925hPa',''),directionKey='wind_direction_925hPa'+suffix;return {speeds:h[speedKey]||[],directions:h[directionKey]||[]}}).filter(function(m){return m.directions.length});return {lat:Number(row.latitude)||point[0],lon:Number(row.longitude)||point[1],times:(h.time||[]).map(function(t){return new Date(t+'+07:00').getTime()}),members:members}}).filter(function(row){return row.members.length});ensembleReady=ensembleReadings.length>0;ensembleTime=ensembleReady&&ensembleReadings[0].times.length?new Date(ensembleReadings[0].times[0]).toLocaleDateString('sv-SE',{timeZone:'Asia/Jakarta'}):''}).catch(function(){ensembleReady=false;ensembleReadings=[]})
  }
