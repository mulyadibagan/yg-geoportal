...
[search,regency,documentFilter].forEach(function(el){el.addEventListener(el===search?'input':'change',render)});legalFilter.addEventListener("change",function(){activeScheme="";render()});document.querySelectorAll('[data-type]').forEach(function(b){b.addEventListener('click',function(){activeType=b.dataset.type;document.querySelectorAll('[data-type]').forEach(function(x){x.classList.toggle('is-active',x===b)});render()})});schemeGrid.addEventListener("click",function(e){var card=e.target.closest("[data-area-action],[data-area-scheme]");if(!card)return;var action=card.dataset.areaAction||"",scheme=card.dataset.areaScheme||"";if(action==="approved"){legalFilter.value="approved";activeScheme=""}else if(action==="process"){legalFilter.value="process";activeScheme=""}else if(scheme){legalFilter.value="approved";activeScheme=activeScheme===scheme?"":scheme}render();document.querySelector(".psd-content").scrollIntoView({behavior:"smooth",block:"start"})});regencyGrid.addEventListener("click",function(e){var card=e.target.closest("[data-regency-card]");if(!card)return;regency.value=card.dataset.regencyCard;render();document.querySelector(".psd-content").scrollIntoView({behavior:"smooth",block:"start"})});init();
})();
