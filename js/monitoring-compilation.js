(function(){
  ...
  var clusterResultSummary=document.getElementById('cluster-result-summary');
  ...
  function monitoringResultHtml(group){
    if(group.unmonitored)return'<div class="cluster-monitoring-result"><b>Belum dimonitor</b><br><small>Belum ada laporan hasil monitoring untuk polygon ini.</small></div>';
    var latest=group.latest||{},metrics=latest.metrics||{},survival=metricValue(metrics,metricDefs[2]),alive=metricValue(metrics,metricDefs[0]),dead=metricValue(metrics,metricDefs[1]),area=metricValue(metrics,metricDefs[6]);
    var rows=[['Tanggal terbaru',fmtDate(latest.date)],['Kondisi',latest.status&&latest.status.label||'—'],['Survival',survival===null?'—':numberFormat(survival)+'%'],['Hidup',alive===null?'—':numberFormat(alive)],['Mati/rusak',dead===null?'—':numberFormat(dead)],['Luas terpantau',area===null?'—':numberFormat(area)+' ha']];
    return'<div class="cluster-monitoring-result"><b>Hasil monitoring terbaru</b><table>'+rows.map(function(row){return'<tr><th>'+esc(row[0])+'</th><td>'+esc(row[1])+'</td></tr>';}).join('')+'</table><small>'+group.history.length+' laporan monitoring</small></div>';
  }
  function renderClusterResultSummary(groups,selected,pending){
    if(!clusterResultSummary)return;
    var monitoredGroups=groups.filter(function(group){return!group.unmonitored;}),alive=0,dead=0,area=0,planted=0,reports=0;
    monitoredGroups.forEach(function(group){var latest=group.latest||{},metrics=latest.metrics||{},live=metricValue(metrics,metricDefs[0]),lost=metricValue(metrics,metricDefs[1]),size=metricValue(metrics,metricDefs[6]);if(live!==null)alive+=live;if(lost!==null)dead+=lost;if(size!==null)area+=size;if(latest.plantedCount!==null&&latest.plantedCount!==undefined)planted+=Number(latest.plantedCount)||0;reports+=(group.history||[]).length;});
    var total=alive+dead,rate=total?alive/total*100:0,aliveWidth=total?alive/total*100:0,deadWidth=total?dead/total*100:0,title=selected.length?selected.join(' · '):'Semua donor, desa, dan fase';
    clusterResultSummary.innerHTML='<div class="cluster-result-head"><div><span class="eyebrow">RINGKASAN HASIL KLASTER</span><h3>Kondisi tanaman pada klaster terpilih</h3><p>Hanya laporan terbaru setiap objek yang dihitung agar histori tidak dijumlahkan berulang.</p></div></div><div class="cluster-result-row"><div class="cluster-result-name"><b>'+esc(title)+'</b><small>'+monitoredGroups.length+' objek dimonitor · '+reports+' laporan'+(pending?' · '+pending+' belum dimonitor':'')+'</small></div><div class="cluster-result-bar" role="img" aria-label="'+esc(numberFormat(rate))+' persen hidup"><span class="alive" style="width:'+aliveWidth+'%"></span><span class="dead" style="width:'+deadWidth+'%"></span></div><div class="cluster-result-rate">'+esc(numberFormat(rate))+'%</div><div class="cluster-result-stats"><span><b>'+esc(numberFormat(planted))+'</b><small>bibit pada objek dimonitor</small></span><span><b>'+esc(numberFormat(alive))+'</b><small>hidup</small></span><span><b>'+esc(numberFormat(dead))+'</b><small>mati/rusak</small></span><span><b>'+esc(numberFormat(area))+' ha</b><small>terpantau</small></span></div></div>';
  }
  ...
  function updateClusterMap(data){
    ...
    renderClusterResultSummary(groups,selected,pending);
    if(clusterMapSummary)clusterMapSummary.innerHTML='<strong>'+mapped+' polygon program</strong><p>'+(selected.length?esc(selected.join(' · ')):'Semua donor, desa, dan fase')+'<br>'+monitored+' sudah dimonitor · '+pending+' belum dimonitor<br>'+esc(numberFormat(totalArea))+' ha area program · '+esc(numberFormat(totalPlanted))+' bibit tertanam.</p><div class="cluster-map-summary-list">'+groups.slice(0,12).map(function(group){var d=clusterDimensions(group),latest=group.latest||{},href=group.unmonitored?'webgis.html?layer=area_mangrove&object='+encodeURIComponent(group.key):'monitoring-detail.html?object='+encodeURIComponent(group.key)+'&title='+encodeURIComponent(group.label||latest.title||'Objek monitoring'),result=group.unmonitored?'Belum dimonitor':fmtDate(latest.date)+' · '+(latest.status&&latest.status.label||'Sudah dimonitor');return'<a href="'+href+'"><b>'+esc(group.label||latest.title||'Objek monitoring')+'</b>'+esc(d.village.label)+' · '+esc(d.phase.label)+'<br>'+esc(result)+'</a>';}).join('')+'</div>';
  }
