(() => {
  ...
  const OFFICIAL_LAYERS = [
    { id: "desa_intervensi", url: "data/desa_intervensi.geojson?v=20260924-17desa-ghimbo" },
  ...
  function canonicalMangroveVillage(props, text) {
    ...
    const normalizedExplicitVillage = explicitVillage.toLowerCase();
    const searchable = [
      normalizedExplicitVillage,
    ...
    const aliases = [
      ...
      ["teluk piyai pesisir", "Teluk Piyai Pesisir"],
      ["telukpiyai pesisir", "Teluk Piyai Pesisir"]
    ];
    const exact = aliases.find(item => normalizedExplicitVillage === item[0]);
    if (exact) return exact[1];
    if (explicitVillage) return explicitVillage;
    const inferred = aliases.find(item => searchable.includes(item[0]));
  ...
  async function renderDashboard(data) {
    ...
    active.forEach(feature => {
      ...
      if (isMangrove) {
        ...
        if (mangroveVillage && layerId === "area_mangrove") {
