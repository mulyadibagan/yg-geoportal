(function () {
  ...
  function alignWithLiveShell() {
    ['css/style.css?v=20260723-revert-layout','css/language-switcher.css?v=20260721-all-pages1','css/navigation-v2.css?v=20260807-mobile-submenu-links1','css/dayun.css?v=20260917-public2'].forEach(function(href){
  ...
  function renderAgroSummary(summary){
    ...
    var mapLayout=document.querySelector('.dy-section-nav')||document.querySelector('.dy-map-layout');if(mapLayout&&mapLayout.previousElementSibling!==target)mapLayout.parentNode.insertBefore(target,mapLayout);
    var all=summary.all;
    target.innerHTML='<div class="dy-summary-head"><div><span>AGROFORESTRI DAYUN</span><h2>Ringkasan Seluruh Blok A–F</h2></div><p>Kebun agroforestri KUPS Rimba Sejahtera, Kampung Dayun.</p></div><div class="dy-summary-kpis">'+[
      ['Luas seluruh blok',fmtArea(all.blockAreaHa)],['Blok','6'],['Gawangan terpetakan',fmtInteger(all.mappedGawangan)],['Jenis komoditas',fmtInteger(all.cropTypes)]
    ].map(function(item){return '<article><small>'+item[0]+'</small><strong>'+item[1]+'</strong></article>';}).join('')+'</div><div class="dy-summary-foot"><span>'+fmtInteger(all.gawanganWithData)+' dari '+fmtInteger(all.mappedGawangan)+' gawangan memiliki informasi tanaman.</span><span>Catatan kegiatan terakhir: <b>'+fmtDate(all.latestRecordDate)+'</b></span>'+(all.missingGawangan.length?'<span class="is-warning">'+fmtInteger(all.missingGawangan.length)+' gawangan belum memiliki data tanaman.</span>':'')+'</div><div class="dy-crop-summary-groups">'+cropGroupHtml(all,window.DayunAgroSummary.MPTS,'Pohon multiguna (MPTS)','pohon')+cropGroupHtml(all,window.DayunAgroSummary.HORTICULTURE,'Hortikultura','tanaman')+'</div>';
    var crops=target.querySelector('.dy-crop-summary-groups'),commodities=document.getElementById('dayun-commodities');if(crops&&commodities){commodities.innerHTML='<div class="dy-section-head"><div><h2>Komoditas kebun</h2></div><p>Pilih jenis tanaman untuk melihat gawangan tempat tumbuhnya.</p></div>';commodities.appendChild(crops);}
    if(pineappleEntry)pineappleEntry.innerHTML='<div class="dy-pineapple-entry-copy"><span>ANALISIS KOMODITAS</span><h2>Nanas Queen Dayun</h2><p>Buka analisis khusus untuk melihat populasi, luas operasional, pemupukan, ethrel, pembungaan, panen, hama dan penyakit, serta prioritas pemeriksaan setiap gawangan.</p><div><small>POPULASI TERCATAT</small><strong>'+fmtInteger(all.pineapplePlants)+' tanaman</strong></div><div><small>BUAH DIPANEN</small><strong>'+fmtInteger(all.pineappleHarvest)+' buah</strong></div><div><small>AKTIVITAS TERAKHIR</small><strong>'+fmtDate(all.latestRecordDate)+'</strong></div></div><a class="dy-pineapple-entry-link" href="dayun-analisis-nanas.html">Buka Analisis Nanas →</a>';
    overview.innerHTML='<div class="dy-section-head"><div><span>RINGKASAN PER BLOK</span><h2>Perbandingan Blok A–F</h2></div><p>Pilih satu blok untuk melihat peta, komposisi tanaman, riwayat panen, dan daftar gawangan penyusunnya.</p></div><div class="dy-block-table-wrap"><table class="dy-block-table"><thead><tr><th>Blok</th><th>Gawangan</th><th>Luas blok</th><th>Luas gawangan</th><th>Hortikultura</th><th>Nanas</th><th>Dipanen</th><th>MPTS</th><th></th></tr></thead><tbody>'+summary.codes.map(function(code){var block=summary.blocks[code];return '<tr><th scope="row">'+block.name+'</th><td>'+fmtInteger(block.gawanganWithData)+' / '+fmtInteger(block.mappedGawangan)+'</td><td>'+fmtArea(block.blockAreaHa)+'</td><td>'+fmtArea(block.gawanganAreaHa)+'</td><td>'+fmtInteger(block.horticulturePlants)+' · '+fmtInteger(block.horticultureTypes)+' jenis</td><td>'+fmtInteger(block.pineapplePlants)+'</td><td>'+fmtInteger(block.pineappleHarvest)+'</td><td>'+fmtInteger(block.mptsPlants)+' · '+fmtInteger(block.mptsTypes)+' jenis</td><td><a href="dayun-blok.html?block='+code+'">Lihat detail →</a></td></tr>';}).join('')+'</tbody></table></div><details class="dy-summary-note"><summary>Tentang luas kebun</summary><p>Luas operasional dihitung satu kali per gawangan dari luas operasional komoditas terbesar, sehingga komoditas yang berada pada ruang tanam yang sama tidak dijumlahkan ganda.</p></details>';
    var headings=Array.from(overview.querySelectorAll('thead th')).map(function(th){return th.textContent;});overview.querySelectorAll('tbody tr').forEach(function(row){Array.from(row.children).forEach(function(cell,index){if(cell.tagName==='TD')cell.setAttribute('data-label',headings[index]);});});
  }

  function loadDayunSpatialData(){
    function json(url){return window.DayunDataSource.fetchJSON(url);}
    return Promise.all([
      json('data/dayun-map.geojson?v=20260916-objectid1'),
      json('data/dayun-context.geojson?v=20260908-1').catch(function(){return {features:[]};}),
      json('data/dayun-gawangan-details.json?v=20260917-performance1'),
      json('data/dayun-blocks.geojson?v=20260916-official1')
    ]);
  }
  ...
  function initDayunMap(data,spatialDataPromise){
    ...
    var intro=document.querySelector('.dy-map-page-intro p');if(intro)intro.textContent='Pilih blok atau gawangan pada peta untuk melihat luas, tanaman, dan rincian kebun.';
    if(typeof L==='undefined'){mapEl.innerHTML='<div class="dy-map-loading">Peta belum dapat ditampilkan. <button type="button" onclick="location.reload()">Coba lagi</button></div>';return;}
    ...
    (spatialDataPromise||loadDayunSpatialData()).then(function(results){
      ...
      if(window.DayunData)window.DayunData.loadDetails().then(function(details){var updated=window.DayunAgroSummary.build(details,geojson,blockGeojson);Object.keys(blockByName).forEach(function(name){blockByName[name].summary=updated.blocks[blockByName[name].code];});renderAgroSummary(updated);}).catch(function(){});
      ...
      var blockLines=L.geoJSON(blockGeojson,{pane:'blockBoundaryPane',style:function(feature){var p=feature.properties||{};return{color:p.color||'#7c3aed',weight:4,opacity:1,fillColor:p.color||'#7c3aed',fillOpacity:.025,lineCap:'round',lineJoin:'round'};},onEachFeature:function(feature,layer){var p=feature.properties||{},blockName=p.name||'Blok',blockInfo=blockByName[blockName]||{},summary=blockInfo.summary||{};layer.bindTooltip(esc(blockName),{permanent:true,direction:'center',className:'dy-block-label'});layer.bindPopup(function(){var summary=blockInfo.summary||{};return '<div class="dy-map-popup"><b>'+esc(blockName)+'</b><div class="dy-map-popup-metrics"><span><small>Luas blok</small><strong>'+fmtArea(p.areaHa)+'</strong></span><span><small>Luas gawangan tanam</small><strong>'+fmtArea(blockInfo.gawanganAreaHa)+'</strong></span><span><small>Gawangan memiliki data</small><strong>'+fmtInteger(summary.gawanganWithData)+' / '+fmtInteger(summary.mappedGawangan)+'</strong></span><span><small>Hortikultura</small><strong>'+fmtInteger(summary.horticulturePlants)+' · '+fmtInteger(summary.horticultureTypes)+' jenis</strong></span><span><small>Nanas tercatat</small><strong>'+fmtInteger(summary.pineapplePlants)+'</strong></span><span><small>Buah dipanen</small><strong>'+fmtInteger(summary.pineappleHarvest)+'</strong></span><span><small>MPTS</small><strong>'+fmtInteger(summary.mptsPlants)+' · '+fmtInteger(summary.mptsTypes)+' jenis</strong></span></div><div class="dy-map-popup-actions"><a href="dayun-blok.html?block='+encodeURIComponent(blockInfo.code)+'">Lihat detail blok →</a></div></div>';});}});
      var blockLayer=L.featureGroup([blockHalo,blockLines]).addTo(map);contextLayers.blocks=blockLayer;
      var overlays={'Blok A–F':blockLayer};if(contextLayers['desa-dayun'])overlays['Kampung Dayun']=contextLayers['desa-dayun'];if(contextLayers['ps-hkm'])overlays['Kawasan HKm']=contextLayers['ps-hkm'];L.control.layers({'Satelit':satellite,'Peta jalan':osm},overlays,{position:'topright'}).addTo(map);
    ...
    }).catch(function(error){console.error(error);mapEl.innerHTML='<div class="dy-map-loading">Peta belum dapat ditampilkan. <button type="button" onclick="location.reload()">Coba lagi</button></div>';});
  }
  ...
  async function initDayunRainHistory(){
    ...
    }catch(error){document.getElementById('dayun-rain-kpis').innerHTML='';var period=document.getElementById('dayun-rain-chart-period');if(period)period.textContent='Periode belum tersedia';document.getElementById('dayun-rain-months').innerHTML='<div class="dy-weather-loading">Riwayat curah hujan sementara belum dapat dimuat.</div>';document.getElementById('dayun-rain-history-note').textContent='Silakan muat ulang halaman beberapa saat lagi.';}
  }
  ...
  if(spatialDataPromise)spatialDataPromise.then(function(r){renderAgroSummary(window.DayunAgroSummary.build(r[2],r[0],r[3]));}).catch(function(){['dayun-agro-summary','dayun-pineapple-entry','dayun-block-overview'].forEach(function(id){var el=document.getElementById(id);if(el)el.innerHTML='<p>Informasi kebun belum dapat ditampilkan. <button type="button" onclick="location.reload()">Coba lagi</button></p>';});});
  ...
  }).catch(function(error){console.error(error);toast(error.message);var el=document.getElementById('dayun-map');if(el)el.innerHTML='<p>Peta belum dapat ditampilkan. <button type="button" onclick="location.reload()">Coba lagi</button></p>';});
})();
