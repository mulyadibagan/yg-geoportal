(function () {
  ...
  function alignWithLiveShell() {
    ['css/style.css?v=20260723-revert-layout','css/language-switcher.css?v=20260721-all-pages1','css/navigation-v2.css?v=20260807-mobile-submenu-links1','css/dayun.css?v=20260916-gawangan1'].forEach(function(href){
  ...
  function fmtNumber(value,unit) { if(value==null||value==='')return 'Belum tersedia';var number=Number(value),label=Number.isFinite(number)?number.toLocaleString('id-ID',{maximumFractionDigits:2}):String(value);return label+(unit?' '+unit:''); }
  function fmtPeriod(value) { return value&&String(value).length===10?fmtDate(String(value)):String(value||'Belum tersedia'); }
  ...
  function compactFacts(items){return '<dl class="dy-gawangan-facts">'+items.filter(function(item){return item[1]!=null&&item[1]!=='';}).map(function(item){return '<div><dt>'+esc(item[0])+'</dt><dd>'+esc(item[2]?fmtNumber(item[1],item[2]):item[1])+'</dd></div>';}).join('')+'</dl>';}
  function activityLine(label,value,unit){return value==null?'':'<span><b>'+esc(label)+'</b>'+esc(fmtNumber(value,unit))+'</span>';}
  function periodList(label,items){if(!items||!items.length)return '';return '<div class="dy-gawangan-history"><b>'+esc(label)+'</b><div>'+items.map(function(item){return '<span><small>'+esc(fmtPeriod(item.period))+'</small><strong>'+esc(fmtNumber(item.count,'btg'))+'</strong></span>';}).join('')+'</div></div>';}
  function cropCard(crop){
    var operations=activityLine('Pemupukan',crop.fertilizedCount,'btg')+activityLine('Insektisida',crop.insecticideCount,'btg')+activityLine('Pestisida',crop.pesticideCount,'btg')+activityLine('Panen buah',crop.harvestFruitCount,'buah')+activityLine('Berat panen',crop.harvestKg,'kg')+activityLine('Bunga nanas',crop.pineappleFlowerCount,'btg')+activityLine('Bibit tersedia',crop.seedlingAvailability,'btg');
    var replant=crop.replantCrop||crop.replantCount!=null?'<p class="dy-gawangan-note"><b>Penyisipan:</b> '+esc([crop.replantCrop,crop.replantCount!=null?fmtNumber(crop.replantCount,'btg'):null].filter(Boolean).join(' · '))+'</p>':'';
    return '<article class="dy-gawangan-crop"><header><h3>'+esc(crop.crop||'Tanaman belum dicatat')+'</h3>'+(crop.notes?'<span>'+esc(crop.notes)+'</span>':'')+'</header>'+compactFacts([['Periode tanam',crop.plantingPeriod],['Umur',crop.ageMonths,'bulan'],['Rotasi',crop.rotation],['Jumlah vegetasi',crop.vegetationCount,'btg'],['Jarak tanam',crop.spacingM2,'m²'],['Dimensi',crop.widthM!=null&&crop.lengthM!=null?fmtNumber(crop.widthM,'m')+' × '+fmtNumber(crop.lengthM,'m'):null],['Luas operasional',crop.operationalAreaHa,'ha']])+replant+(operations?'<div class="dy-gawangan-operations">'+operations+'</div>':'')+periodList('Riwayat panen nanas',crop.pineappleHarvest)+periodList('Riwayat ethrel',crop.ethrel)+'</article>';
  }
  function openGawanganDetail(record,properties){
    var drawer=document.getElementById('dayun-gawangan-detail');if(!drawer)return;
    var p=properties||{},objectId=p.objectId||(record&&record.objectId)||'',title=p.displayName||p.name||('Gawangan '+(record&&record.gawangan||''));
    var body=record&&record.crops&&record.crops.length?record.crops.map(cropCard).join(''):'<div class="dy-gawangan-empty"><b>Data operasional belum tersedia</b><p>Polygon tetap ditampilkan pada peta. Data tanaman untuk gawangan ini belum tercatat pada sumber Excel.</p></div>';
    drawer.innerHTML='<div class="dy-gawangan-drawer-head"><div><small>DATA GAWANGAN PUBLIK</small><h2>'+esc(title)+'</h2><p>'+esc(objectId)+' · Blok '+esc(p.block||(record&&record.block)||'-')+' · Luas polygon '+esc(fmtArea(p.sourceGawanganAreaHa==null?p.areaHa:p.sourceGawanganAreaHa))+'</p></div><button type="button" aria-label="Tutup rincian gawangan">×</button></div><div class="dy-gawangan-source"><span>'+(record?record.crops.length:0)+' jenis tanaman tercatat</span><small>Sumber: DATA GAWANG UP. JUNI · KUPS Rimba Sejahtera</small></div><div class="dy-gawangan-crops">'+body+'</div><footer>Luas operasional tanaman berasal dari lembar kerja dan dapat berbeda dari luas polygon hasil pemetaan.</footer>';
    drawer.hidden=false;document.body.classList.add('dy-gawangan-open');drawer.querySelector('button').addEventListener('click',function(){drawer.hidden=true;document.body.classList.remove('dy-gawangan-open');});
  }
  ...
  function initDayunMap(data){
    ...
    var intro=document.querySelector('.dy-map-page-intro p');if(intro)intro.textContent='Klik polygon gawangan untuk melihat komoditas, periode tanam, jumlah vegetasi, pemupukan, panen, dan catatan yang tersedia.';
    ...
    Promise.all([fetch('data/dayun-map.geojson?v=20260916-objectid1'),fetch('data/dayun-context.geojson?v=20260908-1'),fetch('data/dayun-gawangan-details.json?v=20260916-1')]).then(function(responses){if(responses.some(function(response){return !response.ok;}))throw new Error('Data peta atau rincian gawangan tidak dapat dimuat.');return Promise.all(responses.map(function(response){return response.json();}));}).then(function(results){
      var geojson=results[0],contextGeojson=results[1],gawanganData=results[2],gawanganById={},objectProperties={},groups={},featureLayers={},categoryBounds={},contextLayers={},active=data.layers[0].id;
      ...
      L.geoJSON(geojson,{style:featureStyle,onEachFeature:function(feature,layer){
        ...
        layer.bindPopup('<div class="dy-map-popup"><b>'+esc(publicObjectName(p.displayName||p.name))+'</b>'+blockLine+'<span>'+areaText+'</span>'+partLine+cropLine+'<small>Klik polygon untuk membuka rincian gawangan</small>'+detailLink+'</div>');
        if(p.displayId||p.shortId)layer.bindTooltip(esc(p.displayId||p.shortId),{sticky:true,direction:'top'});
        layer.on('click',function(){document.querySelectorAll('.dy-object-key').forEach(function(x){x.classList.toggle('active',x.getAttribute('data-category')===p.category);});openGawanganDetail(gawanganRecord,p);});
      ...
      function renderLegend(){
        ...
        var picker=legendEl.querySelector('.dy-gawangan-picker select');if(picker)picker.addEventListener('change',function(){var id=picker.value,layers=featureLayers[id]||[];if(!layers.length)return;var bounds=L.featureGroup(layers).getBounds();if(bounds.isValid())map.fitBounds(bounds,{padding:[44,44],maxZoom:19});layers[0].openPopup();openGawanganDetail(gawanganById[id],objectProperties[id]);});
  ...
  function renderDayunRainHistory(payload){
    ...
    var bars=rows.map(function(row,index){var height=row.rain*scale,x=left+index*step+(step-barW)/2,y=base-height;return '<rect x="'+x+'" y="'+y+'" width="'+barW+'" height="'+Math.max(height,row.rain>0?1:0)+'" rx="2" fill="#1687a7"><title>'+new Intl.DateTimeFormat('id-ID',{day:'numeric',month:'short',year:'numeric',timeZone:'Asia/Jakarta'}).format(new Date(row.date+'T12:00:00+07:00'))+': '+dayunNum(row.rain,1)+' mm</title></rect>';}).join('');
    var monthTicks=[],seen={};rows.forEach(function(row,index){var key=row.date.slice(0,7);if(!seen[key]){seen[key]=true;monthTicks.push('<text x="'+(left+index*step)+'" y="211" fill="#52675e" font-size="10" font-weight="700">'+new Intl.DateTimeFormat('id-ID',{month:'short',timeZone:'Asia/Jakarta'}).format(new Date(row.date+'T12:00:00+07:00'))+'</text>');}});
