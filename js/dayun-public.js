(function () {
  ...
  function initDayunMap(data){
    ...
    Promise.all([fetch('data/dayun-map.geojson?v=20260916-gawangan1'),fetch('data/dayun-context.geojson?v=20260908-1')]).then(function(responses){if(!responses[0].ok||!responses[1].ok)throw new Error('GeoJSON peta tidak dapat dimuat.');return Promise.all(responses.map(function(response){return response.json();}));}).then(function(results){
      ...
      L.geoJSON(geojson,{style:featureStyle,onEachFeature:function(feature,layer){
        ...
        layer.bindPopup('<div class="dy-map-popup"><b>'+esc(publicObjectName(p.displayName||p.name))+'</b>'+blockLine+'<span>'+areaText+'</span><small>Agroforestri KUPS Rimba Sejahtera · HKm Mandiri Sejahtera</small>'+detailLink+'</div>');
