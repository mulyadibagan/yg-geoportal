(function () {
  ...
  function initDayunMap(data){
    ...
    Promise.all([fetch('data/dayun-map.geojson?v=20260916-objectid1'),fetch('data/dayun-context.geojson?v=20260908-1'),fetch('data/dayun-blocks.geojson?v=20260916-1')]).then(function(responses){if(responses.some(function(response){return !response.ok;}))throw new Error('GeoJSON peta tidak dapat dimuat.');return Promise.all(responses.map(function(response){return response.json();}));}).then(function(results){
      var geojson=results[0],contextGeojson=results[1],blockGeojson=results[2],groups={},featureLayers={},categoryBounds={},contextLayers={},active=data.layers[0].id;
      ...
      var blockLayer=L.geoJSON(blockGeojson,{style:function(feature){var p=feature.properties||{};return{color:p.color||'#7c3aed',weight:2.5,opacity:.95,fillColor:p.color||'#7c3aed',fillOpacity:.025,dashArray:'9 5'};},onEachFeature:function(feature,layer){var p=feature.properties||{};layer.bindTooltip(esc(p.name||'Blok'),{permanent:true,direction:'center',className:'dy-block-label'});layer.bindPopup('<div class="dy-map-popup"><b>'+esc(p.name||'Blok')+'</b><span>Luas pada peta '+fmtArea(p.areaHa)+'</span><small>Agroforestri KUPS Rimba Sejahtera</small></div>');}}).addTo(map);
      contextLayers['blocks']=blockLayer;
      ...
      L.geoJSON(geojson,{style:featureStyle,onEachFeature:function(feature,layer){
        ...
        layer.bindPopup('<div class="dy-map-popup"><b>'+esc(publicObjectName(p.displayName||p.name))+'</b>'+blockLine+'<span>'+areaText+'</span>'+partLine+'<small>Agroforestri KUPS Rimba Sejahtera · HKm Mandiri Sejahtera</small>'+detailLink+'</div>');
