(function () {
  ...
  function initDayunMap(data){
    ...
    Promise.all([fetch('data/dayun-map.geojson?v=20260916-objectid1'),fetch('data/dayun-context.geojson?v=20260908-1')]).then(function(responses){if(responses.some(function(response){return !response.ok;}))throw new Error('GeoJSON peta tidak dapat dimuat.');return Promise.all(responses.map(function(response){return response.json();}));}).then(function(results){
