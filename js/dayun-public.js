(function () {
  ...
  function renderDayunRainHistory(payload){
    ...
    var monthEntries=Object.entries(monthly),currentMonth=monthEntries[monthEntries.length-1],previousMonth=monthEntries[monthEntries.length-2],comparison=previousMonth&&previousMonth[1]?((currentMonth[1]-previousMonth[1])/previousMonth[1])*100:null;
    var kpis=[['7 hari',dayunNum(sumLast(7),1)+' mm'],['30 hari',dayunNum(sumLast(30),1)+' mm'],['90 hari',dayunNum(sumLast(90),1)+' mm'],['Hari kering beruntun',dryStreak+' hari'],['Hujan tertinggi',dayunNum(peak.rain,1)+' mm'],['Perubahan bulan',comparison==null?'Belum cukup data':(comparison>=0?'+':'')+dayunNum(comparison,0)+'%']];
