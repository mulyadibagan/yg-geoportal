(function(){
  ...
  var groups=[],seededEvidence=[],liveEvidence=[],plantingByVillage={},trainingByVillage={};
  ...
  function reconcileDonor(record){if(String(record.donor||'').trim())return record;var haystack=[record.name,record.location,record.regency,record.target,record.partner,record.topic,record.group].map(function(value){return String(value||'').trim();}).join(' ').toLowerCase();if(/buruk bakul|kelapa pati|sepahat/.test(haystack)&&/mangrove|bakau|nipah|pesisir|ekowisata|produk kreatif|digital communication/.test(haystack))record.donor='Aramco Asia Singapore';return record;}
  function groupForVillage(name){var n=normalize(name);return groups.find(function(g){return normalize(g.village)===n;})||null;}
  function buildPlantingSummary(features){plantingByVillage={};(features||[]).forEach(function(f){var p=f.properties||{},key=normalize(p.Desa);if(!key)return;var row=plantingByVillage[key]||(plantingByVillage[key]={plots:0,areaHa:0,seedlings:0,years:{}});row.plots++;row.areaHa+=Number(p.Luas_Ha)||0;row.seedlings+=Number(p.Jumlah_Bib)||0;if(p.Tahun)row.years[p.Tahun]=true;});}
  function plantingMarkup(name){var row=plantingByVillage[normalize(name)];if(!row)return '<div class="aramco-group aramco-group-empty"><small>PENANAMAN TERPETAKAN</small><strong>Data plot belum tersedia</strong></div>';var years=Object.keys(row.years).sort(),period=years.length>1?years[0]+'–'+years[years.length-1]:(years[0]||'Tahun belum tersedia');return '<div class="aramco-group"><small>PENANAMAN TERPETAKAN</small><strong>'+row.seedlings.toLocaleString('id-ID')+' bibit · '+row.areaHa.toLocaleString('id-ID',{minimumFractionDigits:2,maximumFractionDigits:2})+' ha</strong><div class="aramco-group-meta"><span><b>Jumlah plot</b>'+row.plots+' polygon</span><span><b>Tahun tanam</b>'+esc(period)+'</span></div></div>';}
  function buildTrainingSummary(rows){trainingByVillage={};(rows||[]).forEach(function(source){var r=reconcileDonor(Object.assign({},source));if(!/aramco asia singapore/i.test(r.donor||''))return;var village=['Buruk Bakul','Kelapa Pati','Sepahat','Tanjung Kuras'].find(function(name){return normalize(r.location).indexOf(normalize(name))!==-1;});if(!village)return;var key=normalize(village),row=trainingByVillage[key]||(trainingByVillage[key]={activities:0,male:0,female:0,dates:[]});row.activities++;row.male+=Number(r.male)||0;row.female+=Number(r.female)||0;if(r.date)row.dates.push(r.date);});}
  function trainingMarkup(name){var row=trainingByVillage[normalize(name)],url='capacity-building.html?search='+encodeURIComponent(name);if(!row)return '<div class="aramco-group aramco-group-empty"><small>PELATIHAN TERVERIFIKASI</small><strong>Belum ada laporan Aramco</strong><a class="aramco-group-doc" href="'+url+'">Periksa data pelatihan →</a></div>';row.dates.sort();var total=row.male+row.female,years=row.dates.map(function(d){return String(d).slice(0,4);}).filter(Boolean),period=years.length?(years[0]===years[years.length-1]?years[0]:years[0]+'–'+years[years.length-1]):'Tanggal belum tersedia';return '<div class="aramco-group"><small>PELATIHAN TERVERIFIKASI</small><strong>'+row.activities+' kegiatan · '+total.toLocaleString('id-ID')+' peserta</strong><div class="aramco-group-meta"><span><b>Peserta</b>'+row.male.toLocaleString('id-ID')+' laki-laki · '+row.female.toLocaleString('id-ID')+' perempuan</span><span><b>Periode laporan</b>'+esc(period)+'</span></div><a class="aramco-group-doc" href="'+url+'">Buka laporan pelatihan →</a></div>';}
  ...
  document.addEventListener('DOMContentLoaded',function(){
    Promise.all([
      ...
      fetch(MANGROVE_FILE,{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('mangrove');return r.json();}).catch(function(){return {features:[]};}),
      fetch(TRAINING_FILE,{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('training');return r.json();}).catch(function(){return [];})
    ]).then(function(results){groups=(results[1]&&results[1].groups)||[];buildPlantingSummary(results[2]&&results[2].features);buildTrainingSummary(results[3]);var rows=results[0];var donor=(rows||[]).find(function(d){return d.slug==='aramco'||/Aramco Asia Singapore/i.test(d.name||'');});if(!donor)throw new Error('aramco');seededEvidence=donor.verifiedEvidence||[];render(donor);renderEvidence(combinedEvidence());}).catch(function(error){console.error('Dashboard Aramco gagal dimuat:',error);text('aramco-updated','Data program belum dapat dimuat');});
