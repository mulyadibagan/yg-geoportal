(function(){
  ...
  fetch('data/dayun-gawangan-details.json?v=20260917-all-profile1',{cache:'no-store'}).then(function(response){if(!response.ok)throw Error('Data tidak dapat dimuat.');return response.json();}).then(init).catch(function(error){console.error(error);gawangan.innerHTML='<option value="">Data belum dapat dimuat</option>';setError('Data gawangan belum dapat dimuat. Silakan coba lagi.');});
})();