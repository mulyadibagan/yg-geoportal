(function () {
  ...
  function renderV1ReleaseDossier(dossier) {
    dossier = dossier || {};
    var checks = dossier.validationChecks || [], gates = dossier.releaseGates || [];
    document.getElementById("rdtr-v1-dossier").innerHTML = dossier.id
      ? '<div class="rdtr-program-status">' + statusBadge(dossier.releaseDecision) + '<p>' + esc(dossier.promotionRule) + '</p></div>' +
        '<div class="rdtr-network-kpis"><span><strong>' + number(dossier.openWorkItemCount, 0) + '</strong>pekerjaan terbuka</span><span><strong>' +
        number(dossier.acceptedEvidenceCount, 0) + '</strong>bukti diterima</span><span><strong>' + number(gates.filter(function (gate) { return gate.status === "completed"; }).length, 0) +
        '/' + number(gates.length, 0) + '</strong>gerbang rilis selesai</span></div>' +
        '<div class="rdtr-plan-items">' + checks.map(function (check) { return '<article class="rdtr-plan-item"><header><h4>' + esc(check.id + " · " + check.label) +
        '</h4></header><p>' + esc(check.rule) + '</p></article>'; }).join("") + '</div>' +
        '<div class="rdtr-itbx-scroll"><table><thead><tr><th>Gerbang</th><th>Status</th><th>Keputusan</th><th>Bukti</th><th>Pengambil keputusan</th></tr></thead><tbody>' +
        gates.map(function (gate) { return '<tr><td><strong>' + esc(gate.id) + '</strong><br>' + esc(gate.gate) + '</td><td>' + statusBadge(gate.status) +
        '</td><td>' + statusBadge(gate.decision) + '<br><small>' + esc(gate.note) + '</small></td><td>' + esc((gate.evidenceRefs || []).join(", ") || "belum tercatat") +
        '</td><td>' + esc(gate.decidedBy || "belum ditetapkan") + '</td></tr>'; }).join("") + '</tbody></table></div><p class="rdtr-component-note">' + esc(dossier.disclaimer) + '</p>'
  ...
  function renderFieldObservationSchema(schema) {
    schema = schema || {}; var fields = schema.fields || [];
    document.getElementById("rdtr-field-schema").innerHTML = fields.length
      ? '<div class="rdtr-program-status">' + statusBadge(schema.status) + '<p>' + esc(schema.disclaimer) + '</p></div>' +
        '<div class="rdtr-network-kpis"><span><strong>' + number(fields.length, 0) + '</strong>kolom</span><span><strong>' +
        number(fields.filter(function (field) { return field.required; }).length, 0) + '</strong>wajib</span><span><strong>' +
        number((schema.emptyGeoJson && schema.emptyGeoJson.features || []).length, 0) + '</strong>observasi saat ini</span></div>' +
        '<div class="rdtr-itbx-scroll"><table><thead><tr><th>Kolom</th><th>Tipe</th><th>Wajib</th><th>Definisi</th></tr></thead><tbody>' +
        fields.map(function (field) { return '<tr><td><strong>' + esc(field.name) + '</strong></td><td>' + esc(field.type) + '</td><td>' +
        esc(field.required ? "ya" : "tidak") + '</td><td>' + esc(field.description) + '</td></tr>'; }).join("") +
        '</tbody></table></div><p class="rdtr-component-note">Aturan validasi: ' + esc((schema.validationRules || []).join(" · ")) + '</p>'
      : '<p class="rdtr-component-note">Schema observasi lapangan belum tersedia.</p>';
  }

  function renderRegulationCurrencyAudit(audit) {
    audit = audit || {}; var rows = audit.items || [];
    document.getElementById("rdtr-regulation-audit").innerHTML = rows.length
      ? '<div class="rdtr-program-status">' + statusBadge(audit.status) + '<p>' + esc(audit.rule) + '</p></div>' +
        '<div class="rdtr-network-kpis"><span><strong>' + number(audit.totalRegulations, 0) + '</strong>rujukan</span><span><strong>' +
        number(audit.checkedCount, 0) + '</strong>diperiksa ulang</span><span><strong>' + number(audit.pendingRecheckCount, 0) + '</strong>menunggu pemeriksaan</span></div>' +
        '<div class="rdtr-itbx-scroll"><table><thead><tr><th>ID</th><th>Regulasi</th><th>Status pemeriksaan</th><th>Temuan</th><th>Tanggal</th><th>Legal review</th></tr></thead><tbody>' +
        rows.map(function (row) { return '<tr><td><strong>' + esc(row.regulationRef) + '</strong></td><td><a href="' + esc(row.officialUrl) +
        '" target="_blank" rel="noopener noreferrer">' + esc(row.code) + '</a><br>' + esc(row.title) + '</td><td>' + statusBadge(row.verificationStatus) +
        '</td><td>' + esc(row.finding) + '</td><td>' + esc(row.checkedAt || "belum") + '</td><td>' + esc(row.legalReviewer || "belum direview") +
        '</td></tr>'; }).join("") + '</tbody></table></div><p class="rdtr-component-note">' + esc(audit.disclaimer) + '</p>'
      : '<p class="rdtr-component-note">Audit keberlakuan regulasi belum tersedia.</p>';
  }
