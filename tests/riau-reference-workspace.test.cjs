...
test("Riau reference workspace is staff-only and additive", () => {
  ...
  assert.match(page, /riau-reference-workspace\.js\?v=20260921-opd2/);
...
test("Geoportal layers are filtered by OPD rather than theme", () => {
  ...
  assert.doesNotMatch(page, /id="rg-search"/);
  assert.doesNotMatch(catalog, /el\("rg-theme"\)/);
  assert.doesNotMatch(catalog, /el\("rg-search"\)/);
});
...
test("only staff-ready Geoportal derivatives enter the interactive map", () => {
  ...
  assert.match(workspace, /MAX_ACTIVE_CATALOG_LAYERS = 8/);
