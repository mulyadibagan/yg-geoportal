...
test('interactive reference layer binds and opens a company popup',async()=>{
 ...
 const context={REFERENCE_LAYERS:{test:{type:'oil_palm_company',file:'data/test.geojson',label:'Referensi',version:'test'}},referenceLayerObjects:{},referenceLayerState:{},setStatus(){},fetchReferenceData:async()=>({type:'FeatureCollection',features:[feature]}),mergeReferenceSupplements:async(c,d)=>d,MAP_PANES:{reference:'reference'},referenceRendererFor:()=>({}),referenceStyle:()=>({}),referencePopup:(c,f)=>f.properties.PO_COMPANY,referenceCountInfo:()=>({count:1,featureCount:1,label:'1',statusLabel:'1'}),document:{querySelector:()=>null},L:{DomEvent:{stopPropagation(){}},geoJSON:(data,opts)=>{options=opts;opts.onEachFeature(feature,{bindPopup(v){popup=v},on(event,fn){click=fn},openPopup(){opened=true}});return layer}}};
