...
test("builds an internal baseline for exactly the 11 invited planning-area villages", () => {
  ...
  assert.equal(result.regulationCurrencyAudit.checkedCount, 18);
  assert.equal(result.regulationCurrencyAudit.pendingRecheckCount, 1);
  ...
  assert.equal(result.ygDraftRdtr.regulationCurrencyAudit.checkedCount, 18);
  assert.equal(result.v1AnalyticalBaseline.version, "1.0.0-internal-analytical-baseline");
  assert.equal(result.v1AnalyticalBaseline.zoneConclusionCount, result.map.ygCandidateZones.features.length);
  assert.equal(result.v1AnalyticalBaseline.coveragePct, 100);
  assert.equal(result.v1AnalyticalBaseline.regulationAudit.pending, 1);
  assert.ok(result.v1AnalyticalBaseline.conclusions.every(row => row.conclusion && row.policyDecision && row.evidenceLocks.length >= 7));
  assert.equal(result.ygDraftRdtr.v1AnalyticalBaseline.id, result.v1AnalyticalBaseline.id);
  ...
  assert.match(result.regulationCurrencyAudit.items.find(row => row.regulationRef === "R14").finding, /PP 8\/2026/);
