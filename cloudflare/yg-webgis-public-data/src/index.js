...
const PRIVATE_DATA_ROUTES = {
  ...
  "/api/staff/pptpkh-riau-2023": ["internal/land-reform/pptpkh-riau-2023.geojson", "application/geo+json; charset=utf-8"],
  ...
  "/api/staff/social-forestry-makmur-pesisir-analysis": ["internal/social-forestry/makmur-pesisir-spatial-analysis.json", "application/json; charset=utf-8"],
...
async function privateDataApi(request, env, url) {
  ...
  if (!route && "/api/staff/fire-monthly-report" === url.pathname) {
    ...
    route = [`internal/fire-monthly/${month}.json`, "application/json; charset=utf-8"];
  }
  if (!route && "/api/staff/phl-svlk-monthly-report" === url.pathname) {
    const month = String(url.searchParams.get("month") || "");
    if (!/^20\d{2}-(0[1-9]|1[0-2])$/.test(month)) return staffJson({ ok: false, error: "invalid_month" }, 400);
    route = [`internal/phl-svlk-monthly/${month}.json`, "application/json; charset=utf-8"];
  }
  if (!route) return staffJson({ ok: false, error: "not_found" }, 404);
  try {
...
var index_default = { async fetch(request, env) {
  ...
  if (privateDataApiRoute) return privateDataApi(request, env, url);
