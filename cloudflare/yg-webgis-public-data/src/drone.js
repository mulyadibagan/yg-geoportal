const DRONE_HEADERS = {
  "access-control-allow-origin": "https://webgisyg.id",
...
function newJob(body = {}) {
  ...
  return {
    id,
    title: String(body.title || body.area || "Survei drone").slice(0, 160),
    project: String(body.project || "YG GeoPortal").slice(0, 160),
    sourceType: body.sourceType === "drive" ? "drive" : "upload",
    driveUrl: body.sourceType === "drive" ? String(body.driveUrl || "").trim() : null,
    status: body.sourceType === "drive" ? "pending" : "uploading",
    files: [],
...
async function uploadFile(request, env, id, encodedName) {
  ...
  if (job.sourceType !== "upload" || !["uploading", "pending"].includes(job.status)) return reply({ ok: false, error: "job_not_uploadable" }, 409);
  const name = safeName(decodeURIComponent(encodedName || "photo.jpg"));
  const type = request.headers.get("content-type") || "application/octet-stream";
  if (!/^image\/(jpeg|jpg)$/i.test(type) && !/\.jpe?g$/i.test(name)) return reply({ ok: false, error: "jpeg_only" }, 415);
  const size = Number(request.headers.get("content-length") || 0);
  if (size && size > 95 * 1024 * 1024) return reply({ ok: false, error: "file_too_large" }, 413);
  const key = `drone/uploads/${id}/${name}`;
  await env.PUBLIC_SNAPSHOTS.put(key, request.body, { httpMetadata: { contentType: type, cacheControl: "private, max-age=0" } });
  const files = Array.isArray(job.files) ? job.files.filter((f) => f?.key !== key) : [];
  files.push({ name, key, bytes: size || null });
  job.files = files;
...
async function finalizeUpload(env, id) {
  const job = await readJson(env, jobKey(id));
  if (!job) return reply({ ok: false, error: "job_not_found" }, 404);
  if (job.sourceType !== "upload") return reply({ ok: false, error: "not_upload_job" }, 409);
  if (!Array.isArray(job.files) || job.files.length < 3) return reply({ ok: false, error: "minimum_three_photos" }, 400);
  job.status = "pending";
...
async function serveCog(request, env, id, url) {
  ...
  if (!access || access !== job.accessToken) return reply({ ok: false, error: "unauthorized" }, 401);
  const rangeHeader = request.headers.get("range");
  const object = await env.PUBLIC_SNAPSHOTS.get(job.cogKey, rangeHeader ? { range: request.headers } : undefined);
  if (!object) return reply({ ok: false, error: "cog_missing" }, 404);
  const headers = new Headers(DRONE_HEADERS);
...
export async function handleDroneRequest(request, env, url) {
  ...
  const finalMatch = url.pathname.match(/^\/api\/drone\/jobs\/(drn-[a-zA-Z0-9-]+)\/finalize$/);
  if (finalMatch && request.method === "POST") return finalizeUpload(env, finalMatch[1]);
  const jobMatch = url.pathname.match(/^\/api\/drone\/jobs\/(drn-[a-zA-Z0-9-]+)$/);
